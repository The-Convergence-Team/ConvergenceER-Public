...
$Event(0, Default, function() {
    ...
    //Mimic
    InitializeCommonEvent(0, 20000352, 2047440650);
    InitializeCommonEvent(0, 20000353, 2047440651, 2047440650, 2047442650);
});
...
$Event(2047442500, Restart, function(eventFlagId, assetEntityId, assetEntityId2, objactEventFlag, objactParamId, animationId, animationId2, animationId3) {
    if (EventFlag(eventFlagId)) {
        DisableObjAct(assetEntityId2, objactParamId);
        ReproduceAssetAnimation(assetEntityId, animationId2);
        ReproduceAssetAnimation(assetEntityId2, animationId3);
        EndEvent();
    }
L0:
...
$Event(2047442200, Restart, function(chrEntityId) {
    if (!CharacterHasSpEffect(chrEntityId, 13690)) {
        SetSpEffect(chrEntityId, 13690);
    }
});

$Event(2047442300, Restart, function(chrEntityId) {
    SetSpEffect(chrEntityId, 20007030);
});

$Event(2047442611, Restart, function(eventFlagId, entityId, eventFlagId2, eventFlagId3, eventFlagId4) {
    EndIf(EventFlag(eventFlagId) || !PlayerIsInOwnWorld());
    SetEventFlagID(eventFlagId2, OFF);
    SetEventFlagID(eventFlagId3, OFF);
    WaitFor(ActionButtonInArea(9523, entityId));
    DisplayGenericDialogAndSetEventFlags(108186, PromptType.YESNO, NumberofOptions.TwoButtons, entityId, 3, eventFlagId2, eventFlagId3, eventFlagId3);
...
L1:
    if (!PlayerHasItem(ItemType.Goods, 8186)) {
        ...
        DisplayGenericDialog(308186, PromptType.OKCANCEL, NumberofOptions.NoButtons, entityId, 3);
        WaitFixedTimeSeconds(1);
        RestartEvent();
    }
L2:
    IssueShortWarpRequest(10000, TargetEntityType.Asset, entityId, 191);
    ForceAnimationPlayback(10000, 60810, false, false, false);
    WaitFixedTimeSeconds(2.5);
    SetNetworkconnectedEventFlagID(eventFlagId, ON);
    SetNetworkconnectedEventFlagID(eventFlagId4, ON);
    DisplayGenericDialog(208186, PromptType.OKCANCEL, NumberofOptions.NoButtons, entityId, 3);
    RemoveItemFromPlayer(ItemType.Goods, 8186, 1);
});

$Event(2047442612, Restart, function(eventFlagId, assetEntityId) {
    if (EventFlag(eventFlagId)) {
