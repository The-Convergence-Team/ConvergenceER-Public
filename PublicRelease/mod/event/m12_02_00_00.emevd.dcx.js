// ==EMEVD==
// @docs    er-common.emedf.json
// @compress    DCX_KRAK
// @game    Sekiro
// @string    "N:\\GR\\data\\Param\\event\\common_func.emevd\u0000N:\\GR\\data\\Param\\event\\common_macro.emevd\u0000\u0000\u0000\u0000\u0000\u0000"
// @linked    [0,82]
// @version    3.5
// ==/EMEVD==

// コンストラクタ -- constructor
$Event(0, Default, function() {
    RegisterBonfire(71223, 12021953, 0, 0, 0, 5);
    RegisterBonfire(71224, 12021954, 0, 0, 0, 5);
    RegisterBonfire(71222, 12021955, 0, 0, 0, 5);
    RegisterBonfire(71226, 12021956, 0, 0, 0, 5);
    RegisterBonfire(71227, 12021957, 0, 0, 0, 5);
    RegisterBonfire(71228, 12021958, 0, 0, 0, 5);
    RegisterBonfire(71229, 12021959, 0, 0, 0, 5);
    RegisterBonfire(71291, 12021961, 0, 0, 0, 5);
    InitializeCommonEvent(0, 9005810, 12020850, 71221, 12020951, 12021951, 1084227584); // Mimic Tear / Bear Grace
    InitializeCommonEvent(0, 9005810, 12020800, 71220, 12020950, 12021950, 1084227584);
    InitializeCommonEvent(0, 9005810, 12020805, 12020000, 12020960, 12021960, 1084227584); //guardian grace
    $InitializeEvent(0, 12022503);
    $InitializeEvent(0, 12022580);
    InitializeCommonEvent(0, 90005920, 12020900, 12021900, 12023900);
    InitializeCommonEvent(0, 900005610, 12021682, 100, 800, 0);
    InitializeCommonEvent(0, 900005610, 12021683, 100, 800, 0);
    $InitializeEvent(0, 12022800);
    $InitializeEvent(0, 12022810); //Caimar: Initiation Script
    $InitializeEvent(0, 12022822); //Caimar: 2nd phase
    $InitializeEvent(0, 12022849);
    $InitializeEvent(0, 12023750);
    $InitializeEvent(0, 12023751);
    $InitializeEvent(0, 12023752);
    $InitializeEvent(0, 12022850);
    $InitializeEvent(0, 12022857);
    //InitializeCommonEvent(0, 90005876, 12022850, 12022851, 903320000, 0, 0);
    //InitializeEvent(0, 12022860, 0); // Activation Script Mimic Tear
    //InitializeEvent(0, 12022865, 12020860, 12020850, 63010);
    $InitializeEvent(0, 12022899);
    InitializeCommonEvent(0, 90005501, 12020520, 12021520, 5, 12021520, 1049401521, 12021522, 12020522);
    InitializeCommonEvent(0, 90005620, 12020570, 12021570, 12021571, 12021572, 12022570, 12022571, 12022572);
    $InitializeEvent(0, 12022567, 12020570, 12022570);
    $InitializeEvent(0, 12022569, 12020524, 12020570);
    $InitializeEvent(0, 12020500);
    InitializeCommonEvent(0, 90005620, 12020575, 12021575, 12021576, 0, 12022575, 12022576, 12022577);
    $InitializeEvent(0, 12022568, 12020575, 12021578);
    $InitializeEvent(0, 12020510);
    $InitializeEvent(0, 12022502);
    $InitializeEvent(0, 12022670);
    InitializeCommonEvent(0, 90005605, 12021203, 524, 12022203, 12020000, 12022203, 12022210, 12022211, 0, 0, 0, 0);
    InitializeCommonEvent(0, 90005605, 12021204, 524, 12022204, 12020000, 12022204, 12022212, 12022213, 0, 0, 0, 0);
    InitializeCommonEvent(0, 90005605, 12021205, 524, 12022205, 12020000, 12022205, 12022215, 12022206, 0, 0, 0, 0);
    $InitializeEvent(0, 12022600);
    $InitializeEvent(0, 12022609);
    $InitializeEvent(0, 12022620);
    $InitializeEvent(0, 12022629);
    $InitializeEvent(0, 12022601, 12020600, 12021600, 12021610);
    $InitializeEvent(1, 12022601, 12020601, 12021601, 12021611);
    $InitializeEvent(2, 12022601, 12020602, 12021602, 12021612);
    $InitializeEvent(3, 12022601, 12020603, 12021603, 12021613);
    $InitializeEvent(4, 12022601, 12020604, 12021604, 12021614);
    $InitializeEvent(5, 12022601, 12020605, 12021605, 12021615);
    $InitializeEvent(6, 12022601, 12020606, 12021606, 12021616);
    $InitializeEvent(7, 12022601, 12020607, 12021607, 12021617);
    $InitializeEvent(0, 12022621, 12020620, 12021620, 12021630);
    $InitializeEvent(1, 12022621, 12020621, 12021621, 12021631);
    $InitializeEvent(2, 12022621, 12020622, 12021622, 12021632);
    $InitializeEvent(3, 12022621, 12020623, 12021623, 12021633);
    $InitializeEvent(4, 12022621, 12020624, 12021624, 12021634);
    $InitializeEvent(5, 12022621, 12020625, 12021625, 12021635);
    $InitializeEvent(6, 12022621, 12020626, 12021626, 12021636);
    $InitializeEvent(7, 12022621, 12020627, 12021627, 12021637);
    InitializeCommonEvent(0, 90005250, 12020211, 12022211, 0, -1);
    InitializeCommonEvent(0, 90005261, 12020220, 12022220, 1116471296, 0, -1);
    InitializeCommonEvent(0, 90005261, 12020221, 12022220, 1116471296, 0, -1);
    InitializeCommonEvent(0, 90005300, 12020221, 12020221, 0, 0, -1);
    $InitializeEvent(0, 12022300, 12020300, 12022300, 0);
    $InitializeEvent(1, 12022300, 12020301, 12022300, 0);
    $InitializeEvent(2, 12022300, 12020303, 12022302, 0);
    InitializeCommonEvent(0, 90005250, 12020303, 12022302, 0, 3006);
    $InitializeEvent(3, 12022300, 12025303, 12022303, 0);
    $InitializeEvent(4, 12022300, 12025304, 12022304, 0);
    InitializeCommonEvent(0, 90005250, 12020304, 12022304, 0, -1);
    $InitializeEvent(5, 12022300, 12025305, 12022305, 0);
    InitializeCommonEvent(0, 90005250, 12020310, 12022305, 1073741824, 3011);
    $InitializeEvent(6, 12022300, 12025306, 12022306, 0);
    $InitializeEvent(7, 12022300, 12025307, 12022307, 0);
    $InitializeEvent(8, 12022300, 12025308, 12022308, 0);
    $InitializeEvent(9, 12022300, 12025309, 12022309, 0);
    InitializeCommonEvent(0, 90005251, 12020340, 1086324736, 0, -1);
    InitializeCommonEvent(0, 90005251, 12020341, 1086324736, 0, -1);
    InitializeCommonEvent(0, 90005251, 12020342, 1086324736, 0, -1);
    InitializeCommonEvent(0, 90005251, 12020343, 1086324736, 0, -1);
    InitializeCommonEvent(0, 90005251, 12020344, 1086324736, 0, -1);
    InitializeCommonEvent(0, 90005261, 0, 0, 0, 0, 0);
    $InitializeEvent(10, 12022300, 12025310, 12022310, 0);
    $InitializeEvent(11, 12022300, 12025311, 12022311, 0);
    $InitializeEvent(12, 12022300, 12025312, 12022312, 0);
    $InitializeEvent(13, 12022300, 12020313, 12022313, 0);
    $InitializeEvent(15, 12022300, 12025315, 12022315, 0);
    $InitializeEvent(16, 12022300, 12020316, 12022316, 0);
    InitializeCommonEvent(0, 90005250, 12020316, 12022316, 0, 3006);
    $InitializeEvent(17, 12022300, 12020317, 12022317, 0);
    $InitializeEvent(18, 12022300, 12025318, 12022318, 0);
    $InitializeEvent(19, 12022300, 12020319, 12022319, 0);
    InitializeCommonEvent(0, 90005250, 12020319, 12022319, 0, -1);
    $InitializeEvent(20, 12022300, 12020331, 12022331, 0);
    $InitializeEvent(21, 12022300, 12020334, 12022334, 0);
    $InitializeEvent(22, 12022300, 12020335, 12022334, 0);
    $InitializeEvent(23, 12022300, 12020336, 12022336, 3);
    $InitializeEvent(24, 12022300, 12020337, 12022336, 0);
    InitializeCommonEvent(0, 90005250, 12020336, 12022336, 0, 3006);
    InitializeCommonEvent(0, 90005250, 12020337, 12022336, 0, 3006);
    $InitializeEvent(25, 12022300, 12025324, 12022314, 0);
    $InitializeEvent(26, 12022300, 12025325, 12022314, 0);
    $InitializeEvent(27, 12022300, 12025326, 12022314, 15);
    $InitializeEvent(29, 12022300, 12025329, 12022329, 0);
    InitializeCommonEvent(0, 90005201, 12020376, 30011, 20011, 1082130432, 0, 0, 0, 0, 0);
    InitializeCommonEvent(0, 90005201, 12020377, 30011, 20011, 1082130432, 0, 0, 0, 0, 0);
    InitializeCommonEvent(0, 90005201, 12020378, 30011, 20011, 1088421888, 0, 0, 0, 0, 0);
    InitializeCommonEvent(0, 90005201, 12020379, 30011, 20011, 1088421888, 0, 0, 0, 0, 0);
    InitializeCommonEvent(0, 90005251, 12020348, 1120403456, 0, -1);
    $InitializeEvent(0, 12022350, 12020390, 12025350);
    $InitializeEvent(0, 12022360, 12020390, 12025350);
    $InitializeEvent(0, 12022370);
    $InitializeEvent(0, 12022371);
    InitializeCommonEvent(0, 90005250, 12020390, 12022350, 1077936128, -1);
    $InitializeEvent(0, 12022372);
    $InitializeEvent(0, 12022373);
    $InitializeEvent(1, 12022350, 12020391, 12025351);
    $InitializeEvent(1, 12022360, 12020391, 12025351);
    $InitializeEvent(2, 12022350, 12020392, 12025352);
    $InitializeEvent(2, 12022360, 12020392, 12025352);
    InitializeCommonEvent(0, 90005250, 12020350, 12022350, 1077936128, -1);
    InitializeCommonEvent(0, 90005250, 12020351, 12022350, 0, -1);
    InitializeCommonEvent(0, 90005250, 12020368, 12022366, 0, 3006);
    InitializeCommonEvent(0, 90005250, 12020368, 12022368, 0, -1);
    InitializeCommonEvent(0, 90005860, 12020830, 0, 12020830, 1, 30620, 0);
    InitializeCommonEvent(0, 90005870, 12020830, 904650600, 25);
    InitializeCommonEvent(0, 90005251, 12020471, 1106247680, 0, 3009);
    InitializeCommonEvent(0, 90005300, 12020470, 12020470, 40648, 1069547520, 0);
    InitializeCommonEvent(0, 90005300, 12020472, 12020472, 40630, 1069547520, 0);
    InitializeCommonEvent(0, 90005300, 12020474, 12020474, 40610, 1069547520, 0);
    InitializeCommonEvent(0, 90005300, 12020477, 12020477, 40642, 1069547520, 0);
    InitializeCommonEvent(0, 90005300, 12020479, 12020479, 40646, 1069547520, 0);
    InitializeCommonEvent(0, 90005250, 12020420, 12022420, 0, -1);
    InitializeCommonEvent(0, 90005251, 12020421, 1114636288, 0, -1);
    InitializeCommonEvent(0, 90005251, 12020424, 1120403456, 0, -1);
    InitializeCommonEvent(0, 90005261, 12020422, 12022422, 1110704128, 0, -1);
    InitializeCommonEvent(0, 90005251, 12020424, 1114636288, 0, -1);
    InitializeCommonEvent(0, 90005251, 12020431, 1092616192, 0, -1);
    InitializeCommonEvent(0, 90005300, 12020430, 12020430, 12020435, 1069547520, 0);
    InitializeCommonEvent(0, 90005300, 12020431, 12020431, 12020445, 1069547520, 0);
    InitializeCommonEvent(0, 90005300, 12020434, 12020434, 0, 1069547520, 0);
    InitializeCommonEvent(0, 90005261, 12020440, 12022440, 1097859072, 0, -1);
    InitializeCommonEvent(0, 90005261, 12020442, 12022442, 1097859072, 0, -1);
    $InitializeEvent(0, 12022440, 12020431, 12020440, 12023440);
    $InitializeEvent(1, 12022440, 12020431, 12020441, 12023440);
    $InitializeEvent(2, 12022440, 12020431, 12020442, 12023440);
    $InitializeEvent(3, 12022440, 12020447, 12020444, 12023444);
    $InitializeEvent(4, 12022440, 12020447, 12020445, 12023444);
    $InitializeEvent(5, 12022440, 12020447, 12020446, 12023444);
    $InitializeEvent(6, 12022440, 12020447, 12020448, 12023444);
    $InitializeEvent(7, 12022440, 12020447, 12020449, 12023444);
    InitializeCommonEvent(0, 90005250, 12020450, 12022450, 1069547520, -1);
    InitializeCommonEvent(0, 90005250, 12020451, 12022450, 1073741824, -1);
    InitializeCommonEvent(0, 90005250, 12020452, 12022450, 1077936128, -1);
    InitializeCommonEvent(0, 90005250, 12020453, 12022450, 1075838976, -1);
    InitializeCommonEvent(0, 90005250, 12020454, 12022450, 0, -1);
    InitializeCommonEvent(0, 90005250, 12020455, 12022450, 0, -1);
    InitializeCommonEvent(0, 90005251, 12025461, 1101004800, 0, -1);
    $InitializeEvent(0, 12022419, 12020400, 12020490, 12022400);
    $InitializeEvent(1, 12022400, 12020380, 12020491, 12022401);
    InitializeCommonEvent(0, 90005261, 12020380, 12022411, 1101004800, 0, -1);
    $InitializeEvent(2, 12022400, 12020402, 12020492, 12022402);
    InitializeCommonEvent(0, 90005261, 12020402, 12022412, 1101004800, 0, -1);
    $InitializeEvent(3, 12022400, 12020403, 12020493, 12022403);
    $InitializeEvent(4, 12022400, 12020404, 12020494, 12022404);
    $InitializeEvent(5, 12022400, 12020405, 12020495, 12022405);
    $InitializeEvent(6, 12022400, 12020406, 12020496, 12022406);
    $InitializeEvent(7, 12022400, 12020407, 12020497, 12022407);
    $InitializeEvent(10, 12022400, 12020410, 12020480, 12022410);
    InitializeCommonEvent(0, 90005702, 12020710, 4063, 4060, 4063);
    $InitializeEvent(0, 12020700, 12020710, 12021711, 12021710);
    $InitializeEvent(0, 12020701, 12022716);
    InitializeCommonEvent(0, 90005752, 12021710, 200, 120, 1077936128);
    $InitializeEvent(0, 12023720, 12020720);
    InitializeCommonEvent(0, 90005703, 12020720, 3601, 3602, 12029151, 3603, 3600, 3603, -1);
    InitializeCommonEvent(0, 90005704, 12020720, 3601, 3600, 12029151, 3);
    InitializeCommonEvent(0, 90005702, 12020720, 3603, 3600, 3603);
    $InitializeEvent(0, 12023721, 12021701, 3, 9710, 1034509410);
    $InitializeEvent(0, 12023710, 12020700, 12020701);
    InitializeCommonEvent(0, 90005725, 4800, 4801, 4803, 12029105, 12020705, 1035450701, 12026700);
    InitializeCommonEvent(0, 90005703, 12020705, 4801, 4802, 12029106, 4801, 4800, 4804, 0);
    InitializeCommonEvent(0, 90005704, 12020705, 4801, 4800, 12029106, 3);
    InitializeCommonEvent(0, 90005702, 12020705, 4803, 4800, 4804);
    InitializeCommonEvent(0, 90005706, 12020730, 930023, 0);
    InitializeCommonEvent(0, 90005780, 12020800, 12022160, 12022161, 12020715, 20, 12022716, 12029019, 1, 0);
    InitializeCommonEvent(0, 90005781, 12020800, 12022160, 12022161, 12020715);
    $InitializeEvent(0, 12022699);
    $InitializeEvent(0, 12022698);
    //Erdtree Warps - Enemy, Obj, Guardian
    InitializeCommonEvent(0, 9005995, 12020750, 12020725, 12020805);
    //Erdtree Sentry
    InitializeCommonEvent(0, 90005870, 12020805, 904810603, 18);
    InitializeCommonEvent(0, 90005860, 12020805, 0, 12020805, 0, 30630, 0);
    InitializeCommonEvent(0, 90005251, 12020805, 1101004800, 0, 0);
    $InitializeEvent(0, 12022055);
    //warp scripts
    InitializeCommonEvent(0, 98005307, 12021660, 12, 5, 0, 0, 12051661, 12020434); // Waygate
    InitializeCommonEvent(1, 98005307, 12021665, 12, 2, 0, 0, 12021666, 0); // Waygate
    
    
    //Spiritshaper Caimar Events Start
    
    
    //Caimar: Dummy Enemies
    $InitializeEvent(0, 12020550, 12020861);
    $InitializeEvent(1, 12020550, 12020862);
    $InitializeEvent(2, 12020550, 12020863);
    $InitializeEvent(3, 12020550, 12020864);
    $InitializeEvent(4, 12020550, 12020865);
    $InitializeEvent(5, 12020550, 12020866);
    $InitializeEvent(6, 12020550, 12020867);
    
    //Caimar: Birb Transform Attacks - Enemy, SpEffect, Anim
    $InitializeEvent(0, 12020551, 12020802, 11742, 3016, 3015, 260);
    
    //Caimar: Bear Transform Attacks - Enemy, SpEffect, Anim
    InitializeEvent(0, 12020552, 260, 11743, 3006, 3007);
    
    //Caimar: Transform Teleporting (Birb) - Enemy, SpEffect, Anim, Idle Anim
    $InitializeEvent(0, 12020553, 12020802, 260);
    
    //InitializeEvent(1, 12020553, 12020803, 220);
    $InitializeEvent(0, 12022830, 12020802, 11742);
    
    //Caimar: Trapped (Bear) Failsafe
    InitializeEvent(0, 12022831, 12020803, 11743);
    
    //Caimar: Bear Swap on Grab Success Event Flag Handler
    $InitializeEvent(0, 12020554);
    
    //Caimar: Wolf Summon (Left) - Enemy, SpEffect, Attack Anim 1
    $InitializeEvent(0, 12020555, 12020808, 11748, 3004);
    
    //Caimar: Bear Trap
    $InitializeEvent(0, 12020556, 12020803, 11743); //Bear 1
    $InitializeEvent(1, 12020556, 12020807, 11743); //Bear 2
    
    //Caimar: Transform Teleporting (Bear) - Enemy, SpEffect, Anim, Idle Anim
    InitializeEvent(0, 12020557, 0, 220);
    
    //Caimar: Wolf Summon (Right) - Enemy, SpEffect, Attack Anim 1
    $InitializeEvent(0, 12020558, 12020804, 11741, 3005);
    
    //Caimar: Airborne Throw Camera Adjustment
    InitializeEvent(0, 12020559, 0);
    
    //Caimar: Custom Playzone (Warp Player Out of Arena On Area Reload)
    InitializeCommonEvent(0, 98005309, 12020800, 12022120, 12021208);
    
    //Spiritshaper Caimar Events End
    
    
    //Bridge Crystals - EventFlag, Crystal, Bridge Piece 1, Bridge Piece 2, VFX
    $InitializeEvent(0, 12020610, 12020613, 12023610, 12024610, 12024611, 12024613);
    $InitializeEvent(1, 12020610, 12020614, 12023615, 12024614, 12024615, 12024616);
    $InitializeEvent(2, 12020610, 12020628, 12023620, 12024617, 12024618, 12024619);
    
    // Dungeon Animal Ambush
    $InitializeEvent(0, 12020680, 12020393, 12022393, 3004, 65, 15, 30, 0);
    $InitializeEvent(1, 12020680, 12020394, 12022393, 3004, 65, 15, 30, 0);
    $InitializeEvent(2, 12020680, 12020395, 12022393, 3004, 65, 15, 30, 0);
    $InitializeEvent(3, 12020680, 12020396, 12022393, 3004, 65, 15, 30, 0);
    $InitializeEvent(4, 12020680, 12020398, 12022398, 3031, 78, 5, 5, 0);
    $InitializeEvent(5, 12020680, 12020414, 12022414, 3000, 39, 40, 55, 229);
    $InitializeEvent(6, 12020680, 12020415, 12022414, 3001, 53, 40, 55, 434);
    $InitializeEvent(7, 12020680, 12020416, 12022416, 3000, 39, 40, 55, 434);
    $InitializeEvent(8, 12020680, 12020417, 12022416, 3001, 53, 40, 55, 431);
    $InitializeEvent(9, 12020680, 12020418, 12022416, 3004, 58, 40, 55, 435);
    $InitializeEvent(10, 12020680, 12020435, 12022431, 3016, 80, 15, 30, 0);
    $InitializeEvent(11, 12020680, 12020436, 12022432, 3016, 80, 30, 40, 0);
    $InitializeEvent(12, 12020680, 12020437, 12022433, 3001, 53, 0, 0, 0);
    $InitializeEvent(13, 12020680, 12020438, 12022432, 3016, 80, 60, 70, 0);
    
    // Enemy activation regions 
    InitializeCommonEvent(0, 90005250, 12020397, 12022397, 0, -1);
    InitializeCommonEvent(1, 90005250, 12020408, 12022408, 0, -1);
    InitializeCommonEvent(2, 90005250, 12020409, 12022408, 0, -1);
    InitializeCommonEvent(3, 90005250, 12020410, 12022408, 0, -1);
    InitializeCommonEvent(4, 90005250, 12020411, 12022408, 0, -1);
    InitializeCommonEvent(5, 90005250, 12020412, 12022408, 0, -1);
    InitializeCommonEvent(6, 90005250, 12020413, 12022408, 0, -1);
    InitializeCommonEvent(7, 90005250, 12020425, 12022425, 0, -1);
    InitializeCommonEvent(8, 90005250, 12020426, 12022426, 0, -1);
    InitializeCommonEvent(9, 90005250, 12020427, 12022427, 0, -1);
    InitializeCommonEvent(10, 90005250, 12020428, 12022425, 0, -1);
    InitializeCommonEvent(11, 90005250, 12020429, 12022425, 0, -1);
    
    // NPC stuff
    InitializeCommonEvent(0, 90005300, 12020425, 12020425, 108300, 0, 0);
    InitializeEvent(0, 12020690, 12020428, 435);
    InitializeEvent(1, 12020690, 12020429, 434);
    
    // Lower Siofra waygate to upper
    InitializeCommonEvent(0, 98005307, 12021206, 12, 2, 0, 0, 12021207, 12090800);
    
    // Wolf Stuff -  Enemy, Region, Anim, anim lenght
    $InitializeEvent(0, 12020695, 12020439, 12022434, 3006, 127, 1705);
    $InitializeEvent(1, 12020695, 12020440, 12022435, 3009, 58, 1704);
    $InitializeEvent(2, 12020695, 12020441, 12022436, 3011, 127, 1701);
});

// プリコンストラクタ -- preconstructor
$Event(50, Default, function() {
    SetCharacterBackreadState(12020700, true);
    SetCharacterBackreadState(12020720, true);
    SetCharacterBackreadState(12020730, true);
    DisableAsset(12021710);
    $InitializeEvent(0, 12020519);
    InitializeEvent(0, 12020050, 0);
});

// 協力NPC_ボス部屋入場 -- Cooperating NPC_Boss room entrance
$Event(12022699, Restart, function() {
    WaitFor(EventFlag(12022805));
    if (!InArea(12020715, 12022717)) {
        InitializeCommonEvent(0, 90005784, 12022160, 12022805, 12020715, 12022800, 12022809, 0);
        EndEvent();
    }
L10:
    EndEvent();
});

// 敵壁暫定消去 -- Enemy wall temporary deletion
$Event(12022698, Restart, function() {
    EndIf(EventFlag(12020800));
    WaitFor(EventFlag(12022160));
    DisableHit(12021698);
});

$Event(12022055, Restart, function() {
    DisableCharacter(12020806);
    DisableCharacterCollision(12020806);
    ForceCharacterDeath(12020806, false);
});

// 平原エレベータ_ガーゴイル像で初回起動 -- Plains Elevator_First activation with gargoyle statue
$Event(12022569, Restart, function(eventFlagId, eventFlagId2) {
    DisableNetworkSync();
    EndIf(EventFlag(eventFlagId));
    WaitFor(EventFlag(eventFlagId2));
    if (PlayerIsInOwnWorld()) {
        SetNetworkconnectedEventFlagID(eventFlagId, ON);
    }
    RestartEvent();
});

// ガーゴイル像_封印扉 -- Gargoyle statue_sealed door
$Event(12022568, Default, function(eventFlagId, assetEntityId) {
    DisableNetworkSync();
    if (EventFlag(eventFlagId)) {
        DisableAsset(assetEntityId);
        EndEvent();
    }
L0:
    CreateAssetfollowingSFX(assetEntityId, 101, 806043);
    WaitFor(PlayerIsInOwnWorld() && EventFlag(eventFlagId));
    DeleteAssetfollowingSFX(assetEntityId, true);
    PlaySE(assetEntityId, SoundType.SFX, 90011);
    WaitFixedTimeSeconds(0.5);
    DisableAsset(assetEntityId);
});

// 消費鍵のガーゴイル像_ギミック制御 -- Gargoyle statue of consumption key_gimmick control
$Event(12022567, Default, function(eventFlagId, areaEntityId) {
    DisableNetworkSync();
    WaitFor(InArea(10000, areaEntityId) && EventFlag(eventFlagId));
    SetSpEffect(10000, 4150);
    WaitFixedTimeSeconds(3);
    RestartEvent();
});

// 下層灯火台イベント -- Lower lighthouse event
$Event(12022600, Restart, function() {
    if (EventFlag(12020609)) {
        DeleteAssetfollowingSFX(12021609, true);
        CreateAssetfollowingSFX(12021609, 200, 812600);
        EndEvent();
    }
L0:
    WaitFor(
        EventFlag(12020600)
            && EventFlag(12020601)
            && EventFlag(12020602)
            && EventFlag(12020603)
            && EventFlag(12020604)
            && EventFlag(12020605)
            && EventFlag(12020606)
            && EventFlag(12020607));
    SetNetworkconnectedEventFlagID(12020609, ON);
    DeleteAssetfollowingSFX(12021609, true);
    CreateAssetfollowingSFX(12021609, 200, 812600);
    WaitFixedTimeSeconds(3);
    DisplayGenericDialog(30010, PromptType.OKCANCEL, NumberofOptions.NoButtons, 0, 5);
});

// 下層灯火点火イベント_XX -- Lower level light ignition event_XX
$Event(12022601, Restart, function(eventFlagId, assetEntityId, assetEntityId2) {
    if (EventFlag(eventFlagId)) {
        DeleteAssetfollowingSFX(assetEntityId, true);
        DeleteAssetfollowingSFX(assetEntityId2, true);
        CreateAssetfollowingSFX(assetEntityId, 200, 812070);
        CreateAssetfollowingSFX(assetEntityId, 201, 812070);
        CreateAssetfollowingSFX(assetEntityId2, 200, 812070);
        EndEvent();
    }
L0:
    EnableCharacterCollision(10000);
    if (PlayerIsInOwnWorld()) {
        WaitFor(ActionButtonInArea(9524, assetEntityId) && PlayerIsInOwnWorld());
        DisableCharacterCollision(10000);
        RotateCharacter(10000, assetEntityId, -1, true);
        ForceAnimationPlayback(10000, 60010, false, false, false);
        WaitFixedTimeSeconds(1.3);
        SetNetworkconnectedEventFlagID(eventFlagId, ON);
        DeleteAssetfollowingSFX(assetEntityId, true);
        DeleteAssetfollowingSFX(assetEntityId2, true);
        CreateAssetfollowingSFX(assetEntityId, 200, 812070);
        CreateAssetfollowingSFX(assetEntityId, 201, 812070);
        CreateAssetfollowingSFX(assetEntityId2, 200, 812070);
        WaitFixedTimeSeconds(2);
        EnableCharacterCollision(10000);
        EndEvent();
    }
L10:
    WaitFor(EventFlag(eventFlagId));
    WaitFixedTimeSeconds(1.3);
    DeleteAssetfollowingSFX(assetEntityId, true);
    DeleteAssetfollowingSFX(assetEntityId2, true);
    CreateAssetfollowingSFX(assetEntityId, 200, 812070);
    CreateAssetfollowingSFX(assetEntityId, 201, 812070);
    CreateAssetfollowingSFX(assetEntityId2, 200, 812070);
    EndEvent();
});

// 下層祖霊部屋へワープ -- Warp to the lower ancestral spirit room
$Event(12022609, Default, function() {
    DisableNetworkSync();
    if (PlayerIsInOwnWorld()) {
        if (EventFlag(12020609)) {
            SetNetworkconnectedEventFlagID(12022610, OFF);
            WaitFor(PlayerIsInOwnWorld() && ActionButtonInArea(9525, 12021609));
            SetNetworkconnectedEventFlagID(12022610, ON);
            RotateCharacter(10000, 12021609, -1, true);
            ForceAnimationPlayback(10000, 60010, false, false, false);
            WaitFixedTimeSeconds(0.4);
            SendInvadingPhantomsHome(0);
            WaitFixedTimeSeconds(1);
            WarpCharacterAndCopyFloorWithFadeout(20000, TargetEntityType.Area, 12082400, -1, 0, false, true);
            WaitFixedTimeFrames(1);
            RestartEvent();
        }
L1:
        WaitFor(EventFlag(12020609));
        RestartEvent();
    }
L10:
    EndIf(CharacterType(10000, TargetType.BlackPhantom));
    WaitFor(EventFlag(12022610));
    WarpCharacterAndCopyFloorWithFadeout(20000, TargetEntityType.Area, 12022202, -1, 0, false, true);
    WaitFixedTimeSeconds(5);
    WarpCharacterAndCopyFloorWithFadeout(20000, TargetEntityType.Area, 12082401, -1, 0, false, true);
    RestartEvent();
});

// 上層灯火台イベント -- Upper lighthouse event
$Event(12022620, Restart, function() {
    if (EventFlag(12020629)) {
        DeleteAssetfollowingSFX(12021629, true);
        CreateAssetfollowingSFX(12021629, 200, 812600);
        EndEvent();
    }
L0:
    WaitFor(
        EventFlag(12020620)
            && EventFlag(12020621)
            && EventFlag(12020622)
            && EventFlag(12020623)
            && EventFlag(12020624)
            && EventFlag(12020625));
    SetNetworkconnectedEventFlagID(12020629, ON);
    DeleteAssetfollowingSFX(12021629, true);
    CreateAssetfollowingSFX(12021629, 200, 812600);
    WaitFixedTimeSeconds(3);
    DisplayGenericDialog(30010, PromptType.OKCANCEL, NumberofOptions.NoButtons, 0, 5);
});

// 上層灯火点火イベント_XX -- Upper light ignition event_XX
$Event(12022621, Restart, function(eventFlagId, assetEntityId, assetEntityId2) {
    if (EventFlag(eventFlagId)) {
        DeleteAssetfollowingSFX(assetEntityId, true);
        DeleteAssetfollowingSFX(assetEntityId2, true);
        CreateAssetfollowingSFX(assetEntityId, 200, 812070);
        CreateAssetfollowingSFX(assetEntityId, 201, 812070);
        CreateAssetfollowingSFX(assetEntityId2, 200, 812070);
        EndEvent();
    }
L0:
    if (PlayerIsInOwnWorld()) {
        EnableCharacterCollision(10000);
        WaitFor(ActionButtonInArea(9524, assetEntityId) && PlayerIsInOwnWorld());
        RotateCharacter(10000, assetEntityId, -1, true);
        ForceAnimationPlayback(10000, 60010, false, false, false);
        DisableCharacterCollision(10000);
        WaitFixedTimeSeconds(1.3);
        SetNetworkconnectedEventFlagID(eventFlagId, ON);
        DeleteAssetfollowingSFX(assetEntityId, true);
        DeleteAssetfollowingSFX(assetEntityId2, true);
        CreateAssetfollowingSFX(assetEntityId, 200, 812070);
        CreateAssetfollowingSFX(assetEntityId, 201, 812070);
        CreateAssetfollowingSFX(assetEntityId2, 200, 812070);
        WaitFixedTimeSeconds(2);
        EnableCharacterCollision(10000);
        EndEvent();
    }
L10:
    WaitFor(EventFlag(eventFlagId));
    WaitFixedTimeSeconds(1.3);
    DeleteAssetfollowingSFX(assetEntityId, true);
    DeleteAssetfollowingSFX(assetEntityId2, true);
    CreateAssetfollowingSFX(assetEntityId, 200, 812070);
    CreateAssetfollowingSFX(assetEntityId, 201, 812070);
    CreateAssetfollowingSFX(assetEntityId2, 200, 812070);
    EndEvent();
});

// 上層祖霊部屋へワープ -- Warp to the upper ancestral spirit room
$Event(12022629, Default, function() {
    if (PlayerIsInOwnWorld()) {
        if (EventFlag(12020629)) {
            SetNetworkconnectedEventFlagID(12022630, OFF);
            WaitFor(PlayerIsInOwnWorld() && ActionButtonInArea(9525, 12021629));
            RotateCharacter(10000, 12021629, -1, true);
            ForceAnimationPlayback(10000, 60010, false, false, false);
            SetNetworkconnectedEventFlagID(12022630, ON);
            WaitFixedTimeSeconds(0.4);
            if (!HasMultiplayerState(MultiplayerState.Singleplayer)) {
                if (EventFlag(12090800)) {
                    SendAllPhantomsHome();
                } else {
                    SendInvadingPhantomsHome(0);
                }
            }
            WaitFixedTimeSeconds(2);
            WarpCharacterAndCopyFloorWithFadeout(20000, TargetEntityType.Area, 12092400, -1, 0, false, true);
            WaitFixedTimeFrames(1);
            RestartEvent();
        }
L1:
        WaitFor(EventFlag(12020629));
        RestartEvent();
    }
L10:
    EndIf(CharacterType(10000, TargetType.BlackPhantom));
    WaitFor(EventFlag(12022630));
    WarpCharacterAndCopyFloorWithFadeout(20000, TargetEntityType.Area, 12022209, -1, 0, false, true);
    WaitFixedTimeSeconds(5);
    WarpCharacterAndCopyFloorWithFadeout(20000, TargetEntityType.Area, 12092401, -1, 0, false, true);
    RestartEvent();
});

// マリカ遺跡２入り口へのワープ -- Warp to Marika Ruins 2 entrance
$Event(12022670, Default, function() {
    if (!ThisEventSlot()) {
        DeleteAssetfollowingSFX(12021670, true);
        CreateAssetfollowingSFX(12021670, 200, 806870);
    }
    SetEventFlagID(12022670, OFF);
    SetEventFlagID(12022671, OFF);
    if (PlayerIsInOwnWorld()) {
        WaitFor(PlayerIsInOwnWorld() && ActionButtonInArea(9140, 12021670));
        //DisplayGenericDialogAndSetEventFlags(4300, PromptType.YESNO, NumberofOptions.TwoButtons, 12021670, 3, 12022670, 12022671, 12022671);
        //if (!EventFlag(12022670)) {
        //    WaitFixedTimeSeconds(1);
        //    RestartEvent();
        //}
L6:
        SetNetworkconnectedEventFlagID(12022670, ON);
        SendInvadingPhantomsHome(0);
        RotateCharacter(10000, 12021670, -1, true);
        ForceAnimationPlayback(10000, 60490, false, false, false);
        WaitFixedTimeSeconds(3);
        WarpCharacterAndCopyFloorWithFadeout(20000, TargetEntityType.Area, 12022670, -1, 10000, false, true);
        RestartEvent();
    }
L10:
    WaitFor(EventFlag(12022670));
    WaitFixedTimeSeconds(3);
    WarpCharacterAndCopyFloorWithFadeout(20000, TargetEntityType.Area, 12022671, -1, 10000, false, false);
    RestartEvent();
});

// 梯子登録 -- ladder registration
$Event(12022580, Restart, function() {
    RegisterLadder(12020580, 12020581, 12021580);
    RegisterLadder(12020582, 12020583, 12021582);
    RegisterLadder(12020584, 12020585, 12021584);
    DisableCharacter(12020806);
});

// 馬禁止領域用SFX -- SFX for horse prohibited area
$Event(12022680, Restart, function() {
    CreateAssetfollowingSFX(12021682, 100, 800);
    CreateAssetfollowingSFX(12021683, 100, 800);
});

// 領域_特殊効果解除_XX -- Area_Special effect cancellation_XX
$Event(12022200, Restart, function(areaEntityId, chrEntityId, spEffectId) {
    EndIf(ThisEventSlot());
    WaitFor(
        ((CharacterType(10000, TargetType.BlackPhantom) && CharacterHasSpEffect(10000, 3710))
            || CharacterType(10000, TargetType.Alive)
            || CharacterType(10000, TargetType.GrayPhantom)
            || CharacterType(10000, TargetType.WhitePhantom))
            && InArea(10000, areaEntityId));
    ClearSpEffect(chrEntityId, spEffectId);
});

// マリカスライム参戦 -- Marika slime participation
$Event(12022404, Restart, function() {
    EndIf(ThisEventSlot());
    WaitFor(
        CharacterAIState(12020404, AIStateType.Combat)
            || CharacterAIState(12020494, AIStateType.Combat));
    ChangeCharacterPatrolBehavior(12020405, 12023405);
    SetNetworkconnectedThisEventSlot(ON);
    EndEvent();
});

// 祖霊信仰者_出現_XX -- Ancestral Spirit Believer_Appearance_XX
$Event(12022300, Restart, function(chrEntityId, areaEntityId, timeSeconds) {
    if (ThisEventSlot()) {
        EnableCharacter(chrEntityId);
        EndEvent();
    }
    DisableCharacter(chrEntityId);
    WaitFor(
        ((CharacterType(10000, TargetType.BlackPhantom) && CharacterHasSpEffect(10000, 3710))
            || CharacterType(10000, TargetType.Alive)
            || CharacterType(10000, TargetType.GrayPhantom)
            || CharacterType(10000, TargetType.WhitePhantom))
            && InArea(10000, areaEntityId));
    WaitFixedTimeSeconds(timeSeconds);
    EnableCharacter(chrEntityId);
    EndEvent();
});

// 祖霊信仰者_バフ掛け_XX -- Ancestral Spirit Believer_Buffing_XX
$Event(12022350, Restart, function(chrEntityId, chrEntityId2) {
    WaitFor(
        EntityInRadiusOfEntity(chrEntityId, 10000, 60, 1) && CharacterHasSpEffect(chrEntityId, 5080));
    WaitFixedTimeFrames(30);
    SetSpEffect(chrEntityId2, 13190);
    WaitFixedTimeFrames(1);
    RestartEvent();
});

// 祖霊信仰者_バフ解除_XX -- Ancestral Spirit Believer_Buff Removal_XX
$Event(12022360, Restart, function(chrEntityId, chrEntityId2) {
    WaitFor(
        EntityInRadiusOfEntity(chrEntityId, 10000, 60, 1)
            && !CharacterHasSpEffect(chrEntityId, 5080));
    ClearSpEffect(chrEntityId2, 13190);
    WaitFixedTimeFrames(30);
    RestartEvent();
});

// 祖霊信仰者_踊りだす -- Ancestor spirit believer_begins to dance
$Event(12022370, Restart, function() {
    EndIf(ThisEventSlot());
    WaitFor(
        CharacterAIState(12020350, AIStateType.Combat)
            || CharacterAIState(12020351, AIStateType.Combat)
            || CharacterAIState(12020390, AIStateType.Combat));
    ForceAnimationPlayback(12020390, 30003, true, false, false);
    EndEvent();
});

// 祖霊信仰者_踊りやめ -- Ancestral spirit believers_stop dancing
$Event(12022371, Restart, function() {
    EndIf(SpecialStandbyEndedFlag(12020390));
    chrSp = (CharacterType(10000, TargetType.BlackPhantom) && CharacterHasSpEffect(10000, 3710))
        || CharacterType(10000, TargetType.Alive)
        || CharacterType(10000, TargetType.GrayPhantom)
        || CharacterType(10000, TargetType.WhitePhantom);
    sp = CharacterHasSpEffect(12020390, 5080) || CharacterHasSpEffect(12020390, 5450);
    sp2 = CharacterHasSpEffect(12020390, 481)
        && !CharacterHasSpEffect(12020390, 90100)
        && !CharacterHasSpEffect(12020390, 90110)
        && !CharacterHasSpEffect(12020390, 90160);
    sp3 = CharacterHasSpEffect(12020390, 482)
        && !CharacterHasSpEffect(12020390, 90100)
        && !CharacterHasSpEffect(12020390, 90120)
        && !CharacterHasSpEffect(12020390, 90160)
        && !CharacterHasSpEffect(12020390, 90162);
    sp4 = CharacterHasSpEffect(12020390, 483)
        && !CharacterHasSpEffect(12020390, 90100)
        && !CharacterHasSpEffect(12020390, 90140)
        && !CharacterHasSpEffect(12020390, 90160)
        && !CharacterHasSpEffect(12020390, 90161);
    sp5 = CharacterHasSpEffect(12020390, 484)
        && !CharacterHasSpEffect(12020390, 90100)
        && !CharacterHasSpEffect(12020390, 90130)
        && !CharacterHasSpEffect(12020390, 90161)
        && !CharacterHasSpEffect(12020390, 90162);
    sp6 = CharacterHasSpEffect(12020390, 487)
        && !CharacterHasSpEffect(12020390, 90100)
        && !CharacterHasSpEffect(12020390, 90150)
        && !CharacterHasSpEffect(12020390, 90160);
    areaChrSp = EntityInRadiusOfEntity(10000, 12020390, 6, 1)
        && CharacterBackreadStatus(12020390)
        && sp
        && chrSp;
    WaitFor(
        areaChrSp
            || HasDamageType(12020390, 0, DamageType.Unspecified)
            || CharacterHasStateInfo(12020390, 436)
            || CharacterHasStateInfo(12020390, 2)
            || CharacterHasStateInfo(12020390, 5)
            || CharacterHasStateInfo(12020390, 6)
            || CharacterHasStateInfo(12020390, 260)
            || CharacterDead(12020351)
            || sp2
            || sp3
            || sp4
            || sp5
            || sp6);
    WaitFixedTimeSeconds(0.1);
    SetSpecialStandbyEndedFlag(12020390, ON);
    if (!(!CharacterHasSpEffect(12020390, 5080) && !CharacterHasSpEffect(12020390, 5450))) {
        WaitFixedTimeSeconds(0);
        ForceAnimationPlayback(12020390, 20003, true, false, false);
    }
L0:
    EndEvent();
});

// 祖霊信仰者_踊り中央 -- Ancestor Spirit Believers_Odori Chuo
$Event(12022372, Restart, function() {
    EndIf(SpecialStandbyEndedFlag(12020391));
    ForceAnimationPlayback(12020391, 30003, true, false, false);
    chrSp = (CharacterType(10000, TargetType.BlackPhantom) && CharacterHasSpEffect(10000, 3710))
        || CharacterType(10000, TargetType.Alive)
        || CharacterType(10000, TargetType.GrayPhantom)
        || CharacterType(10000, TargetType.WhitePhantom);
    sp = CharacterHasSpEffect(12020391, 5080) || CharacterHasSpEffect(12020391, 5450);
    sp2 = CharacterHasSpEffect(12020391, 481)
        && !CharacterHasSpEffect(12020391, 90100)
        && !CharacterHasSpEffect(12020391, 90110)
        && !CharacterHasSpEffect(12020391, 90160);
    sp3 = CharacterHasSpEffect(12020391, 482)
        && !CharacterHasSpEffect(12020391, 90100)
        && !CharacterHasSpEffect(12020391, 90120)
        && !CharacterHasSpEffect(12020391, 90160)
        && !CharacterHasSpEffect(12020391, 90162);
    sp4 = CharacterHasSpEffect(12020391, 483)
        && !CharacterHasSpEffect(12020391, 90100)
        && !CharacterHasSpEffect(12020391, 90140)
        && !CharacterHasSpEffect(12020391, 90160)
        && !CharacterHasSpEffect(12020391, 90161);
    sp5 = CharacterHasSpEffect(12020391, 484)
        && !CharacterHasSpEffect(12020391, 90100)
        && !CharacterHasSpEffect(12020391, 90130)
        && !CharacterHasSpEffect(12020391, 90161)
        && !CharacterHasSpEffect(12020391, 90162);
    sp6 = CharacterHasSpEffect(12020391, 487)
        && !CharacterHasSpEffect(12020391, 90100)
        && !CharacterHasSpEffect(12020391, 90150)
        && !CharacterHasSpEffect(12020391, 90160);
    areaChrSp = EntityInRadiusOfEntity(10000, 12020391, 6, 1)
        && CharacterBackreadStatus(12020391)
        && sp
        && chrSp;
    chr = CharacterDead(12020360) && CharacterDead(12020353) && CharacterDead(12020354);
    WaitFor(
        areaChrSp
            || HasDamageType(12020391, 0, DamageType.Unspecified)
            || CharacterHasStateInfo(12020391, 436)
            || CharacterHasStateInfo(12020391, 2)
            || CharacterHasStateInfo(12020391, 5)
            || CharacterHasStateInfo(12020391, 6)
            || CharacterHasStateInfo(12020391, 260)
            || chr
            || sp2
            || sp3
            || sp4
            || sp5
            || sp6);
    WaitFixedTimeSeconds(0.1);
    SetNetworkconnectedThisEventSlot(ON);
    SetSpecialStandbyEndedFlag(12020391, ON);
    if (!(!CharacterHasSpEffect(12020391, 5080) && !CharacterHasSpEffect(12020391, 5450))) {
        WaitFixedTimeSeconds(0);
        ForceAnimationPlayback(12020391, 20003, true, false, false);
    }
L0:
    EndEvent();
});

// 祖霊信仰者_踊り最後 -- Ancestor spirit believers_last dance
$Event(12022373, Restart, function() {
    EndIf(SpecialStandbyEndedFlag(12020392));
    ForceAnimationPlayback(12020392, 30003, true, false, false);
    chrSp = (CharacterType(10000, TargetType.BlackPhantom) && CharacterHasSpEffect(10000, 3710))
        || CharacterType(10000, TargetType.Alive)
        || CharacterType(10000, TargetType.GrayPhantom)
        || CharacterType(10000, TargetType.WhitePhantom);
    sp = CharacterHasSpEffect(12020392, 5080) || CharacterHasSpEffect(12020392, 5450);
    sp2 = CharacterHasSpEffect(12020392, 481)
        && !CharacterHasSpEffect(12020392, 90100)
        && !CharacterHasSpEffect(12020392, 90110)
        && !CharacterHasSpEffect(12020392, 90160);
    sp3 = CharacterHasSpEffect(12020392, 482)
        && !CharacterHasSpEffect(12020392, 90100)
        && !CharacterHasSpEffect(12020392, 90120)
        && !CharacterHasSpEffect(12020392, 90160)
        && !CharacterHasSpEffect(12020392, 90162);
    sp4 = CharacterHasSpEffect(12020392, 483)
        && !CharacterHasSpEffect(12020392, 90100)
        && !CharacterHasSpEffect(12020392, 90140)
        && !CharacterHasSpEffect(12020392, 90160)
        && !CharacterHasSpEffect(12020392, 90161);
    sp5 = CharacterHasSpEffect(12020392, 484)
        && !CharacterHasSpEffect(12020392, 90100)
        && !CharacterHasSpEffect(12020392, 90130)
        && !CharacterHasSpEffect(12020392, 90161)
        && !CharacterHasSpEffect(12020392, 90162);
    sp6 = CharacterHasSpEffect(12020392, 487)
        && !CharacterHasSpEffect(12020392, 90100)
        && !CharacterHasSpEffect(12020392, 90150)
        && !CharacterHasSpEffect(12020392, 90160);
    areaChrSp = EntityInRadiusOfEntity(10000, 12020392, 6, 1)
        && CharacterBackreadStatus(12020392)
        && sp
        && chrSp;
    chr = CharacterDead(12020370) && CharacterDead(12020371) && CharacterDead(12020372);
    WaitFor(
        areaChrSp
            || HasDamageType(12020392, 0, DamageType.Unspecified)
            || CharacterHasStateInfo(12020392, 436)
            || CharacterHasStateInfo(12020392, 2)
            || CharacterHasStateInfo(12020392, 5)
            || CharacterHasStateInfo(12020392, 6)
            || CharacterHasStateInfo(12020392, 260)
            || chr
            || sp2
            || sp3
            || sp4
            || sp5
            || sp6);
    WaitFixedTimeSeconds(0.1);
    SetNetworkconnectedThisEventSlot(ON);
    SetSpecialStandbyEndedFlag(12020392, ON);
    if (!(!CharacterHasSpEffect(12020392, 5080) && !CharacterHasSpEffect(12020392, 5450))) {
        WaitFixedTimeSeconds(0);
        ForceAnimationPlayback(12020392, 20003, true, false, false);
    }
L0:
    EndEvent();
});

// マリカスライム_NPC変身_XX -- Marika slime_NPC transformation_XX
$Event(12022400, Restart, function(chrEntityId, chrEntityId2, areaEntityId) {
    if (CharacterDead(chrEntityId2)) {
        DisableCharacter(chrEntityId);
    }
    if (ThisEventSlot() && !PlayerIsInOwnWorld()) {
        DisableCharacter(chrEntityId);
        DisableCharacterCollision(chrEntityId);
        EndEvent();
    }
L0:
    if (PlayerIsInOwnWorld()) {
        SetThisEventSlot(OFF);
    }
    DisableCharacter(chrEntityId2);
    DisableCharacterCollision(chrEntityId2);
    WaitFor(CharacterHasSpEffect(chrEntityId, 16307));
    EnableCharacter(chrEntityId2);
    EnableCharacterCollision(chrEntityId2);
    DisableCharacterCollision(chrEntityId);
    WaitFixedTimeFrames(5);
    WarpCharacterAndCopyFloor(chrEntityId2, TargetEntityType.Character, chrEntityId, 900, chrEntityId);
    ForceAnimationPlayback(chrEntityId2, 63010, false, false, true);
    SetSpEffect(chrEntityId2, 16316);
    WaitFixedTimeSeconds(4.4);
    DisableCharacter(chrEntityId);
    DisableCharacterCollision(chrEntityId);
    DisableCharacterAI(chrEntityId);
    EnableCharacterAI(chrEntityId2);
    SetCharacterHome(chrEntityId2, areaEntityId);
});

// マリカスライム_トロル変身_XX -- Marika Slime_Troll Transformation_XX
$Event(12022419, Restart, function(chrEntityId, chrEntityId2, areaEntityId) {
    if (CharacterDead(chrEntityId2)) {
        DisableCharacter(chrEntityId);
    }
    if (ThisEventSlot() && !PlayerIsInOwnWorld()) {
        DisableCharacter(chrEntityId);
        DisableCharacterCollision(chrEntityId);
        EndEvent();
    }
L0:
    if (PlayerIsInOwnWorld()) {
        SetThisEventSlot(OFF);
    }
    DisableCharacter(chrEntityId2);
    DisableCharacterCollision(chrEntityId2);
    WaitFor(CharacterHasSpEffect(chrEntityId, 16309));
    EnableCharacter(chrEntityId2);
    EnableCharacterCollision(chrEntityId2);
    DisableCharacterCollision(chrEntityId);
    WaitFixedTimeFrames(10);
    WarpCharacterAndCopyFloor(chrEntityId2, TargetEntityType.Character, chrEntityId, 900, chrEntityId);
    ForceAnimationPlayback(chrEntityId2, 20015, false, false, true);
    SetSpEffect(chrEntityId2, 16316);
    WaitFixedTimeSeconds(5.2);
    DisableCharacter(chrEntityId);
    DisableCharacterCollision(chrEntityId);
    DisableCharacterAI(chrEntityId);
    SetCharacterHome(chrEntityId2, areaEntityId);
});

// バディ汎用_参戦_XX -- Buddy general purpose_participation_XX
$Event(12022440, Restart, function(chrEntityId, chrEntityId2, patrolInformationEntityId) {
    WaitFor(CharacterAIState(chrEntityId, AIStateType.Combat));
    RequestCharacterAIReplan(chrEntityId2);
    SetSpEffect(chrEntityId2, 5000);
    ChangeCharacterPatrolBehavior(chrEntityId2, patrolInformationEntityId);
});

// 石棺滝登り_滝下 -- Sarcophagus waterfall climbing_Takishita
$Event(12022502, Default, function() {
    DisableNetworkSync();
    WaitFor(EventFlag(12020800) && PlayerIsInOwnWorld() && ActionButtonInArea(9710, 12021502));
    SetEventFlagID(12020502, ON);
    WaitFixedTimeSeconds(0.1);
    SetEventFlagID(9021, ON);
    SetPlayerRespawnPoint(12032502);
    SaveRequest();
    PlayCutsceneToPlayerAndWarp(12020000, CutscenePlayMode.Skippable, 12032502, 12030000, 10000, 12020000, false);
    PlayCutsceneToPlayer(12020001, CutscenePlayMode.SkippableWithFadeOutSkip, 10000);
    WaitFixedTimeFrames(1);
    WaitFixedTimeSeconds(1);
    DisplayAreaWelcomeMessage(12030);
});

// 石棺への導き -- Guide to the sarcophagus
$Event(12022503, Default, function() {
    if (EventFlag(12020502)) {
        DeleteAssetfollowingSFX(12021950, false);
        EndEvent();
    }
    WaitFor(EventFlag(12020800));
    DeleteAssetfollowingSFX(12021950, true);
    CreateAssetfollowingSFX(12021950, 100, 6400);
});

// エレベータ起動 -- elevator start
$Event(12020510, Default, function() {
    InitializeCommonEvent(0, 90005500, 12020520, 12020521, 5, 12021520, 1049401521, 1049403521, 12021522, 12023522, 12022521, 12022522, 12020522, 12022523, 12020524);
    InitializeCommonEvent(0, 90005500, 12020525, 12020526, 4, 12021525, 1045371526, 1045373526, 12021527, 12023527, 1045372526, 12022527, 12020527, 12020528, 0);
});

// エレベータ初期フラグ -- Elevator initial flag
$Event(12020519, Default, function() {
    EndIf(ThisEventSlot());
    SetEventFlagID(12020510, OFF);
    SetEventFlagID(12020520, OFF);
    SetEventFlagID(12020525, ON);
    SetEventFlagID(12020530, ON);
});

// ガーゴイル像解放する前にレバーをロックする -- Lock the lever before releasing the gargoyle statue
$Event(12020500, Default, function() {
    EndIf(EventFlag(12020570));
    WaitFixedTimeSeconds(0.5);
    DisableObjAct(1049401521, 239020);
    WaitFor(EventFlag(12020570));
    EnableObjAct(1049401521, 239020);
});

// 前座ボス撃破 -- Defeat the undercard boss
$Event(12022850, Restart, function() {
    EndIf(EventFlag(12020850));
    WaitFor(CharacterHPValue(12020850) <= 0); // || CharacterHPValue(12020860) <= 0);
    WaitFixedTimeSeconds(4);
    PlaySE(12020850, SoundType.SFX, 888880000);
    WaitFor(CharacterDead(12020850)); // || CharacterDead(12020860));
    ForceCharacterDeath(12020850, true);
    WaitFixedTimeFrames(1);
    HandleBossDefeatAndDisplayBanner(12020850, TextBannerType.GreatEnemyFelled);
    SetEventFlagID(12020850, ON);
    SetEventFlagID(12025865, ON); //music handler eventflag
    SetEventFlagID(12022855, OFF);
    SetEventFlagID(9134, ON);
    if (PlayerIsInOwnWorld()) {
        SetEventFlagID(91134, ON);
    }
    WaitFixedTimeSeconds(6);
    SetEventFlagID(12025865, OFF); //music handler eventflag
});

//Bear Boss activation
$Event(12022857, Restart, function() {
    DisableCharacter(12020860);
    DisableCharacterCollision(12020860);
    if (EventFlag(12020850)) {
        DisableCharacter(12020850);
        DisableCharacterCollision(12020850);
        ForceCharacterDeath(12020850, false);
        EndEvent();
    }
L0:
    DisableCharacterAI(12020850);
    EnableCharacterInvincibility(12020850);
    WaitFor(InArea(10000, 12022851));
    DisableCharacterInvincibility(12020850);
    WaitFixedTimeSeconds(1.5);
L2:
    EnableCharacterAI(12020850);
    ForceAnimationPlayback(12020850, 12002, false, true, false);
    RotateCharacter(12020850, 10000, 3007, false);
    SetNetworkUpdateRate(12020850, true, CharacterUpdateFrequency.AlwaysUpdate);
    DisplayBossHealthBar(Enabled, 12020850, 0, 903320000);
    WaitFor(CharacterDead(12020850));
    DisplayBossHealthBar(Disabled, 12020850, 0, 903320000);
    
});

// 前座ボス出現 -- Undercard boss appears
$Event(12022860, Restart, function() {
    if (EventFlag(12020850)) {
        DisableCharacter(12020850);
        DisableCharacterCollision(12020850);
        ForceCharacterDeath(12020850, false);
        DisableCharacter(12020860);
        DisableCharacterCollision(12020860);
        ForceCharacterDeath(12020860, false);
        EndEvent();
    }
L0:
    EndIf(ThisEventSlot() && !PlayerIsInOwnWorld());
    if (PlayerIsInOwnWorld()) {
        SetThisEventSlot(OFF);
    }
    DisableCharacterAI(12020850);
    DisableCharacterAI(12020860);
    EnableCharacterInvincibility(12020850);
    EnableCharacterInvincibility(12020860);
    if (!EventFlag(12020851)) {
        WaitFor(
            (PlayerIsInOwnWorld() && InArea(10000, 12022851))
                || HasDamageType(12020860, 0, DamageType.Unspecified)
                || CharacterHasStateInfo(12020860, 436)
                || CharacterHasStateInfo(12020860, 2)
                || CharacterHasStateInfo(12020860, 5)
                || CharacterHasStateInfo(12020860, 6)
                || CharacterHasStateInfo(12020860, 260));
        SetNetworkconnectedEventFlagID(12020851, ON);
        if (PlayerIsInOwnWorld()) {
            IssueBossRoomEntryNotification();
            SetNetworkUpdateAuthority(12025850, AuthorityLevel.Forced);
        }
L15:
        NoOp();
    } else {
L1:
        WaitFor(EventFlag(12022855) && InArea(10000, 12022850));
        if (PlayerIsInOwnWorld()) {
            IssueBossRoomEntryNotification();
            SetNetworkUpdateAuthority(12025850, AuthorityLevel.Forced);
        }
L16:
        NoOp();
    }
L2:
    WaitFixedTimeFrames(1);
    SetCharacterAIId(12020850, 90603100);
    DisableCharacterInvincibility(12020860);
    EnableCharacterAI(12020860);
    ForceAnimationPlayback(12020860, 20010, false, false, false);
    if (PlayerIsInOwnWorld()) {
        CopyPlayerCharacterData(10000, 12020850);
    }
    SetNetworkUpdateRate(12020850, true, CharacterUpdateFrequency.AlwaysUpdate);
    SetCharacterHome(12020850, 12022852);
    WaitFixedTimeSeconds(8);
    DisableCharacter(12020860);
    DisableCharacterAI(12020860);
    DisableCharacterInvincibility(12020850);
    EnableCharacterAI(12020850);
    if (!CharacterDead(12020850)) {
        DisplayBossHealthBar(Enabled, 12020850, 0, 903320000);
    }
    SetNetworkconnectedEventFlagID(12022858, ON);
    SetNetworkconnectedThisEventSlot(ON);
});

// コピースライムNPC_ワープ出現 -- Copy Slime NPC_Warp Appears
$Event(12022865, Restart, function(chrEntityId, chrEntityId2, areaEntityId) {
    if (ThisEventSlot() && !PlayerIsInOwnWorld()) {
        DisableCharacter(chrEntityId);
        DisableCharacterCollision(chrEntityId);
        EndEvent();
    }
L0:
    if (PlayerIsInOwnWorld()) {
        SetThisEventSlot(OFF);
    }
    DisableCharacter(chrEntityId2);
    DisableCharacterCollision(chrEntityId2);
    WaitFor(CharacterHasSpEffect(chrEntityId, 16307));
    EnableCharacter(chrEntityId2);
    EnableCharacterCollision(chrEntityId2);
    SetCharacterHome(chrEntityId2, areaEntityId);
    DisableCharacterCollision(chrEntityId);
    WaitFixedTimeFrames(15);
    WarpCharacterAndCopyFloor(chrEntityId2, TargetEntityType.Character, chrEntityId, 900, chrEntityId);
    ForceAnimationPlayback(chrEntityId2, 63010, false, false, true);
    SetSpEffect(chrEntityId2, 16316);
    WaitFixedTimeSeconds(4);
    DisableCharacter(chrEntityId);
    DisableCharacterCollision(chrEntityId);
    DisableCharacterAI(chrEntityId);
});

// ホストがボス部屋入場_スライムボス用 -- Host enters boss room_for slime boss
$Event(12022869, Restart, function(eventFlagId, entityId, areaEntityId, eventFlagId2, chrEntityId, actionButtonParameterId, eventFlagId3, areaEntityId2) {
    if (!EventFlag(eventFlagId)) {
        WaitFixedTimeFrames(1);
        if (eventFlagId3 != 0) {
            GotoIf(L0, EventFlag(eventFlagId3));
            if (areaEntityId2 != 0) {
                areaFlag |= InArea(10000, areaEntityId2);
            }
            areaFlag |= EventFlag(eventFlagId3);
            WaitFor((areaFlag && PlayerIsInOwnWorld()) || EventFlag(eventFlagId));
            RestartIf(EventFlag(eventFlagId));
            Goto(L1);
        }
L0:
        if (PlayerIsInOwnWorld()) {
            WaitFor(
                EventFlag(eventFlagId)
                    || (PlayerIsInOwnWorld()
                        && !EventFlag(eventFlagId)
                        && ActionButtonInArea(actionButtonParameterId, entityId)));
            GotoIf(L2, !PlayerIsInOwnWorld());
            RestartIf(EventFlag(eventFlagId));
            if (!CharacterHasSpEffect(10000, 4250)) {
                RotateCharacter(10000, areaEntityId, 60060, true);
            } else {
                RotateCharacter(10000, areaEntityId, 60060, false);
            }
        }
L3:
        GotoIf(L1, EventFlag(eventFlagId2));
        time = ElapsedSeconds(3);
        WaitFor(
            ((InArea(10000, areaEntityId) || time)
                && PlayerIsInOwnWorld()
                && !EventFlag(eventFlagId))
                || EventFlag(eventFlagId));
        RestartIf(EventFlag(eventFlagId));
        RestartIf(time.Passed);
L1:
        if (PlayerIsInOwnWorld()) {
            SendInvadingPhantomsHome(0);
        }
L2:
        ActivateMultiplayerdependantBuffs(chrEntityId);
        SetNetworkconnectedEventFlagID(eventFlagId2, ON);
        EndIf(!PlayerIsInOwnWorld());
        RestartEvent();
    }
L10:
    EndIf(!PlayerIsInOwnWorld());
    WaitFor(
        PlayerIsInOwnWorld()
            && EventFlag(eventFlagId)
            && (HasMultiplayerState(MultiplayerState.Invasion)
                || HasMultiplayerState(MultiplayerState.InvasionPending))
            && ActionButtonInArea(10000, entityId));
    RotateCharacter(10000, areaEntityId, 60060, true);
    SendInvadingPhantomsHome(0);
    RestartEvent();
});

// 前座ボス_イベント起動 -- Undercard boss_event activation
$Event(12022899, Restart, function() {
    //InitializeEvent(0, 12022869, 12020850, 12021850, 12022850, 12022855, 12025850, 10000, 0, 12022851);
    InitializeCommonEvent(0, 9005800, 12020850, 12021850, 12022850, 12022855, 12025850, 10000, 0, 12022851);
    InitializeCommonEvent(0, 9005801, 12020850, 12021850, 12022850, 12022855, 12022856, 10000);
    InitializeCommonEvent(0, 9005811, 12020850, 12021850, 8, 0);
    InitializeCommonEvent(0, 9005812, 12020850, 12021851, 8, 0, 0);
    InitializeCommonEvent(0, 9005822, 12020850, 391600, 12022855, 12022856, 0, 12022852, 0, 0);
});

// 本ボス撃破 -- Defeat the main boss
$Event(12022800, Restart, function() {
    EndIf(EventFlag(12020800));
    WaitFor(CharacterHPValue(12020800) <= 0);
    WaitFixedTimeSeconds(4);
    PlaySE(12020800, SoundType.SFX, 888880000);
    WaitFor(CharacterDead(12020800));
    HandleBossDefeatAndDisplayBanner(12020800, TextBannerType.LegendFelled);
    ForceAnimationPlayback(12021521, 1, false, false, false);
    SetEventFlagID(12020800, ON);
    SetEventFlagID(12025840, ON); //music handler death event start
    SetEventFlagID(12022802, OFF);
    SetEventFlagID(12022803, OFF);
    SetEventFlagID(12022805, OFF);
    SetEventFlagID(12022814, OFF); //Music Handler phase 1 track end
    SetBossBGM(392500, BossBGMState.Stop1);
    SetEventFlagID(9110, ON);
    if (PlayerIsInOwnWorld()) {
        SetEventFlagID(61110, ON);
    }
    WaitFixedTimeSeconds(6);
    SetEventFlagID(12025840, OFF); //music handler death event end
});

//Caimar Initiation Script
$Event(12022810, Restart, function() {
    if (EventFlag(12020800) || CharacterDead(12020800)) {
        DisableCharacter(12020800);
        DisableCharacter(12020808);
        DisableCharacter(12020804);
        DisableCharacter(12020821);
        DisableCharacterCollision(12020800);
        DisableCharacterCollision(12020808);
        DisableCharacterCollision(12020804);
        DisableCharacterCollision(12020821);
        ForceCharacterDeath(12020800, false);
        ForceCharacterDeath(12020808, false);
        ForceCharacterDeath(12020802, false);
        ForceCharacterDeath(12020803, false);
        ForceCharacterDeath(12020804, false);
        ForceCharacterDeath(12020807, false);
        ForceCharacterDeath(12020821, false);
        ReproduceAssetAnimation(12021521, 1);
        EndEvent();
    }
L0:
    DisableCharacterAI(12020800);
    DisableCharacter(12020800);
    SetNetworkUpdateAuthority(12025801, AuthorityLevel.Forced); //Seamless
    SetNetworkconnectedEventFlagID(12022815, OFF); //Bear Swap Primer Flag
    SetNetworkconnectedEventFlagID(12022816, OFF); //Bear Transform Teleport/Failsafe Primed Flag
    EnableCharacterInvincibility(12020800);
    EnableCharacterInvincibility(12020821);
    SetNetworkUpdateRate(12020800, true, CharacterUpdateFrequency.AlwaysUpdate);
    ForceAnimationPlayback(12020800, 3037, true, false, false);
    DisableLockOnPoint(12020821, 220);
    WaitFor(PlayerIsInOwnWorld() && InArea(10000, 12022820));
    SetNetworkconnectedEventFlagID(12020801, ON);
L2:
    SetEventFlagID(12022814, ON); //Music Handler phase 1 track start
    WaitFixedTimeSeconds(2);
    ForceAnimationPlayback(12020821, 3009, false, false, false);
    WaitFor(CharacterHasSpEffect(12020821, 11745));
    DisableCharacterInvincibility(12020800);
    EnableCharacter(12020800);
    ForceAnimationPlayback(12020800, 3037, true, false, false);
    EnableCharacterAI(12020800);
    IssueShortWarpRequest(12020800, TargetEntityType.Character, 12020821, 511);
    ForceAnimationPlayback(12020800, 20001, false, false, false);
    SetEventFlagID(12022807, ON);
    DisplayBossHealthBar(Enabled, 12020800, 0, 904770002);
    SetNetworkconnectedThisEventSlot(ON);
    WaitFixedTimeFrames(5);
    DisableCharacter(12020821);
});

//Caimar Trapped (Birb) Failsafe
$Event(12022830, Restart, function(entityId, spEffectId) {
    EndIf(EventFlag(12020800));
    DisableNetworkSync();
    WaitFor(
        CharacterHasSpEffect(12020800, spEffectId)
            && InArea(12020800, 12022807)
            && InArea(entityId, 12022807));
    timerExpired = ElapsedSeconds(1.5);
    WaitFor(!InArea(12020800, 12022807) || !InArea(entityId, 12022807) || timerExpired);
    RestartIf(!timerExpired.Passed);
    ForceAnimationPlayback(12020800, 3037, true, false, false);
    WaitFixedTimeFrames(1);
    IssueShortWarpRequest(12020800, TargetEntityType.Area, 12022808, -1);
    WaitFixedTimeFrames(1);
    ForceAnimationPlayback(12020800, 20001, false, false, false);
    RestartEvent();
});

//Caimar: Trapped (Bear) Failsafe
$Event(12022831, Restart, function(X0_4, X4_4) {
    EndIf(EventFlag(12020800));
    DisableNetworkSync();
    WaitFor(
        (CharacterHasSpEffect(12020800, X4_4)
            && InArea(12020800, 12022807)
            && InArea(12020807, 12022807))
            || (CharacterHasSpEffect(12020800, X4_4)
                && InArea(12020800, 12022807)
                && InArea(12020803, 12022807)));
    if (EventFlag(12022816)) { //Bear Transform Teleport/Failsafe Primed Flag
        //WaitFor(CharacterHasSpEffect(12020800, X4_4) && InArea(12020800, 12022807) && InArea(12020807, 12022807));
        timerExpired |= ElapsedSeconds(1.5);
        WaitFor(!InArea(12020800, 12022807) || !InArea(12020807, 12022807) || timerExpired);
        RestartIf(!timerExpired.Passed);
    } else {
        //WaitFor(CharacterHasSpEffect(12020800, X4_4) && InArea(12020800, 12022807) && InArea(12020803, 12022807));
        timerExpired |= ElapsedSeconds(1.5);
        WaitFor(!InArea(12020800, 12022807) || !InArea(12020803, 12022807) || timerExpired);
        RestartIf(!timerExpired.Passed);
    }
    ForceAnimationPlayback(12020800, 3037, true, false, false);
    WaitFixedTimeFrames(1);
    IssueShortWarpRequest(12020800, TargetEntityType.Area, 12022808, -1);
    WaitFixedTimeFrames(1);
    ForceAnimationPlayback(12020800, 20001, false, false, false);
    RestartEvent();
});

// 本ボス_２体判定イベント -- This boss_2 body judgment event
$Event(12022821, Restart, function() {
    EndIf(EventFlag(12020800));
    WaitFor(EventFlag(12022820) && (CharacterDead(12020800) || CharacterDead(12020808)));
    ClearSpEffect(12020800, 17648);
    ClearSpEffect(12020808, 17648);
});

//Caimar 2nd phase
$Event(12022822, Restart, function() {
    EndIf(EventFlag(12020800));
    WaitFor(
        HPRatio(12020800) < 0.65
            && !CharacterHasSpEffect(12020800, 18480)
            && PlayerIsInOwnWorld() //Seamless
            && (HasDamageType(12020800, 0, DamageType.Unspecified)
                || CharacterHasSpEffect(12020800, 52)));
    ForceAnimationPlayback(12020800, 3027, false, true, false);
    WaitFor(CharacterHasSpEffect(12020800, 19955));
    SetEventFlagID(12022814, OFF); //Music Handler phase 1 track end
    SetEventFlagID(12022803, ON); //Music Handler phase transition track start?
    WaitFor(CharacterHasSpEffect(12020800, 11740));
    SetEventFlagID(12022803, OFF); //Music Handler phase transition track end?
    SetEventFlagID(12022802, ON); //Music Handler phase 2 track start?
    WaitFixedTimeFrames(183);
    RequestCharacterAIReplan(12020800);
});

// 本ボス_イベント起動 -- Main boss_event activation
$Event(12022849, Restart, function() {
    InitializeCommonEvent(0, 9005811, 12020800, 12021801, 5, 12020801);
    InitializeCommonEvent(0, 9005822, 12020800, 392500, 12022811, 12022806, 0, 12022802, 0, 0);
    InitializeCommonEvent(0, 9005800, 12020800, 12021800, 12022800, 12022811, 12025800, 10000, 12020801, 12022801);
    InitializeCommonEvent(0, 9005801, 12020800, 12021800, 12022800, 12022811, 12022806, 10000);
    InitializeCommonEvent(0, 9005811, 12020800, 12021800, 3, 12020801);
    InitializeCommonEvent(0, 9005822, 12020800, 392500, 12022807, 12022806, 0, 12022802, 0, 0);
});

// NPC319ダニー_NPC初期化イベント -- NPC319 Danny_NPC initialization event
$Event(12020700, Restart, function(chrEntityId, assetEntityId, assetEntityId2) {
    DisableNetworkSync();
    WaitFixedTimeFrames(1);
    if (PlayerIsInOwnWorld()) {
        if (EventFlag(4060)) {
            SetEventFlagID(12029002, OFF);
        }
    }
L10:
    if (!EventFlag(4065)) {
        DisableCharacter(chrEntityId);
        SetCharacterBackreadState(chrEntityId, true);
        DisableAsset(assetEntityId);
        DisableAsset(assetEntityId2);
        WaitFor(EventFlag(4065));
        RestartEvent();
    }
L5:
    GotoIf(L0, EventFlag(4060));
    GotoIf(L2, EventFlag(4062));
    GotoIf(L3, EventFlag(4063));
L0:
    EnableCharacter(chrEntityId);
    SetCharacterBackreadState(chrEntityId, false);
    ForceAnimationPlayback(chrEntityId, 90107, false, false, false);
    EnableAsset(assetEntityId);
    EnableAsset(assetEntityId2);
    Goto(L20);
L2:
    EnableCharacter(chrEntityId);
    SetCharacterBackreadState(chrEntityId, false);
    SetCharacterTeamType(chrEntityId, TeamType.HostileNPC);
    DisableAsset(assetEntityId);
    DisableAsset(assetEntityId2);
    Goto(L20);
L3:
    ForceCharacterTreasure(chrEntityId);
    DisableCharacter(chrEntityId);
    SetCharacterBackreadState(chrEntityId, true);
    DisableAsset(assetEntityId);
    DisableAsset(assetEntityId2);
    Goto(L20);
L20:
    WaitFor(!EventFlag(4065));
    RestartEvent();
});

// NPC319ダニー_白霊召喚リアルタイム状態遷移 -- NPC319 Danny_White Spirit Summon Real-time state transition
$Event(12020701, Restart, function(entityId) {
    EndIf(!PlayerIsInOwnWorld());
    EndIf(AnyBatchEventFlags(4066, 4077));
    WaitFor(
        EventFlag(4065)
            && EventFlag(12029016)
            && EventFlag(4048)
            && EntityInRadiusOfEntity(entityId, 10000, 10, 1));
    SetEventFlagID(4078, ON);
});

// しゃべスラ_ダミー_NPC初期化イベント -- Chatsura_dummy_NPC initialization event
$Event(12023710, Restart, function(chrEntityId, chrEntityId2) {
    WaitFixedTimeFrames(1);
    DisableCharacter(chrEntityId);
    SetCharacterBackreadState(chrEntityId, true);
    DisableCharacter(chrEntityId2);
    SetCharacterBackreadState(chrEntityId2, true);
});

// しゃべスラ_聖杯に接近 -- Shabesura_Approaching the Holy Grail
$Event(12023711, Restart, function(entityId, eventFlagId) {
    EndIf(!PlayerIsInOwnWorld());
    EndIf(EventFlag(3503));
    EndIf(EventFlag(12029060));
    WaitFor(EntityInRadiusOfEntity(10000, entityId, 5, 1));
    SetEventFlagID(eventFlagId, ON);
    WaitFor(!EntityInRadiusOfEntity(10000, entityId, 5, 1));
    SetEventFlagID(eventFlagId, OFF);
    RestartEvent();
});

// NPC216狼頭の戦士_初期化イベント_地下迷宮で探索 -- NPC216 Wolf-headed Warrior_Initialization event_Explore in the underground labyrinth
$Event(12023720, Restart, function(chrEntityId) {
    DisableNetworkSync();
    WaitFixedTimeFrames(1);
    if (PlayerIsInOwnWorld()) {
        if (EventFlag(3600)) {
            SetEventFlagID(12029152, OFF);
        }
    }
L19:
    if (!EventFlag(3610)) {
        DisableCharacter(chrEntityId);
        SetCharacterBackreadState(chrEntityId, true);
        WaitFor(EventFlag(3610));
        RestartEvent();
    }
L10:
    GotoIf(L0, EventFlag(3600));
    GotoIf(L1, EventFlag(3601));
    GotoIf(L3, EventFlag(3603));
L0:
    EnableCharacter(chrEntityId);
    SetCharacterBackreadState(chrEntityId, false);
    ForceAnimationPlayback(chrEntityId, 930010, false, false, false);
    Goto(L20);
L1:
    EnableCharacter(chrEntityId);
    SetCharacterBackreadState(chrEntityId, false);
    SetCharacterTeamType(chrEntityId, TeamType.HostileNPC);
    Goto(L20);
L3:
    DisableCharacter(chrEntityId);
    SetCharacterBackreadState(chrEntityId, true);
    Goto(L20);
L20:
    WaitFor(!EventFlag(3610));
    RestartEvent();
});

// NPC106ラニ_仲間になるまで秘宝入手不可 -- NPC106 Rani_Unable to obtain secret treasure until you become a friend
$Event(12023721, Restart, function(entityId, targetDistance, spEffectId, eventFlagId) {
    DisableNetworkSync();
    EndIf(!PlayerIsInOwnWorld());
    WaitFor(EventFlag(eventFlagId) && EntityInRadiusOfEntity(10000, entityId, targetDistance, 1));
    SetSpEffect(10000, spEffectId);
    WaitFixedTimeSeconds(3);
    RestartEvent();
});

//Black Nox boss scripts
$Event(12023750, Restart, function() {
    EndIf(EventFlag(12020870));
    WaitFor(CharacterHPValue(12020870) <= 0);
    WaitFixedTimeSeconds(4);
    PlaySE(12020870, SoundType.SFX, 888880000);
    WaitFor(CharacterDead(12020870));
    HandleBossDefeatAndDisplayBanner(12020870, TextBannerType.GreatEnemyFelled);
    AwardItemsIncludingClients(10360);
    SetEventFlagID(12020870, ON);
    SetEventFlagID(12020870, ON);
    if (PlayerIsInOwnWorld()) {
        SetEventFlagID(12020870, ON);
    }
    EndEvent();
});

$Event(12023751, Restart, function() {
    if (EventFlag(12020870)) {
        DisableCharacter(12020870);
        DisableCharacterCollision(12020870);
        ForceCharacterDeath(12020870, false);
        EndEvent();
    }
L0:
    DisableCharacterAI(12020870);
    EnableCharacterInvincibility(12020870);
    DisableLockOnPoint(12020870, 220);
    WaitFixedTimeFrames(3);
    WaitFor(
        PlayerIsInOwnWorld()
            && (InArea(10000, 12022871)
                || HasDamageType(12020870, 10000, DamageType.Unspecified)
                || CharacterHasStateInfo(12020870, 436)
                || CharacterHasStateInfo(12020870, 2)
                || CharacterHasStateInfo(12020870, 5)
                || CharacterHasStateInfo(12020870, 6)
                || CharacterHasStateInfo(12020870, 260)));
    DisableCharacterInvincibility(12020870);
    WaitFor(
        ElapsedSeconds(3)
            || HasDamageType(12020870, 10000, DamageType.Unspecified)
            || EntityInRadiusOfEntity(12020870, 10000, 8, 1));
    EnableCharacterAI(12020870);
    SetNetworkUpdateRate(12020870, true, CharacterUpdateFrequency.AlwaysUpdate);
    DisplayBossHealthBar(Enabled, 12020870, 0, 902500602);
    EnableLockOnPoint(12020870, 220);
});

$Event(12023752, Restart, function() {
    InitializeCommonEvent(0, 9005800, 12020870, 12021870, 12022870, 12022872, 12025870, 10000, 0, 0);
    InitializeCommonEvent(0, 9005801, 12020870, 12021870, 12022870, 12022872, 12022873, 10000);
    InitializeCommonEvent(0, 9005811, 12020870, 12021870, 0, 0);
    InitializeCommonEvent(0, 9005811, 12020870, 12021871, 4, 0);
    InitializeCommonEvent(0, 9005811, 12020870, 12021872, 18, 0);
    InitializeCommonEvent(0, 9005822, 12020870, 921200, 12022872, 12022873, 0, 0, 0, 0);
});

//Spiritshaper Caimar Events 550-558

//Dummy Enemies
$Event(12020550, Restart, function(chrEntityId) {
    DisableCharacter(chrEntityId);
    DisableCharacterHPBarDisplay(chrEntityId);
    DisableCharacterAI(chrEntityId);
    DisableCharacterGravity(chrEntityId);
    EnableCharacterImmortality(chrEntityId);
    EnableCharacterInvincibility(chrEntityId);
    SetCharacterBackreadState(chrEntityId, false);
});

//Transform Attacks (Birb) - Enemy, SpEffect, Anim, Idle Anim
$Event(12020551, Restart, function(chrEntityId, spEffectId, animationId, animationId2, dummypolyId) {
    DisableCharacter(chrEntityId);
    EndIf(EventFlag(12020800));
    DisableNetworkSync();
    DisableLockOnPoint(chrEntityId, 220);
    EnableCharacterImmortality(chrEntityId);
    if (CharacterHasSpEffect(12020800, 11746)) {
        WarpCharacterAndCopyFloor(12020800, TargetEntityType.Area, 12022808, -1, 12020803); //Failsafe if Caimar doesn't warp back into his arena
    }
    WaitFor(
        CharacterHasSpEffect(12020800, spEffectId)
            && !CharacterHasSpEffect(chrEntityId, 11744));
    //DisableLockOnPoint(12020800, 220);
    DisableCharacterGravity(12020800);
    DisableCharacterCollision(12020800);
    DisableCharacterAI(12020800);
    EnableCharacter(chrEntityId);
    SetNetworkUpdateRate(chrEntityId, true, CharacterUpdateFrequency.AlwaysUpdate);
    WarpCharacterAndCopyFloor(chrEntityId, TargetEntityType.Character, 12020800, 260, 12020800);
    ForceAnimationPlayback(chrEntityId, animationId, false, false, true);
    //WaitFixedTimeFrames(1);
    ForceAnimationPlayback(12020800, 3037, false, false, true);
    
    WaitFor(CharacterHasSpEffect(chrEntityId, 11744));
    WaitFor(!CharacterHasSpEffect(chrEntityId, 11744)); //Seamless breaks here
    ForceAnimationPlayback(12020800, 3025, false, false, false);
    //EnableLockOnPoint(12020800, 220);
    EnableCharacterCollision(12020800);
    EnableCharacterGravity(12020800);
    EnableCharacterAI(12020800);
    IssueShortWarpRequest(12020800, TargetEntityType.Character, chrEntityId, dummypolyId);
    RequestCharacterAIReplan(12020800);
    
    ForceAnimationPlayback(chrEntityId, animationId2, false, false, false);
    RestartEvent();
});

//Transform Attacks (Bear) - Enemy, SpEffect, Anim, Idle Anim
//Bear 1 Entity Id (12020803)
//Bear 2 Entity Id (12020807)
//Uses two bears due to unresolved throw issue
$Event(12020552, Restart, function(X0_4, X4_4, X8_4, X12_4) {
    DisableCharacter(12020803);
    DisableCharacter(12020807);
    EndIf(EventFlag(12020800));
    DisableNetworkSync();
    DisableLockOnPoint(12020803, 220);
    DisableLockOnPoint(12020807, 220);
    EnableCharacterImmortality(12020803);
    EnableCharacterImmortality(12020807);
    if (CharacterHasSpEffect(12020800, 11746)) {
        WarpCharacterAndCopyFloor(12020800, TargetEntityType.Area, 12022808, -1, 12020803); //Failsafe if Caimar doesn't warp back into his arena
    }
    
    WaitFor(CharacterHasSpEffect(12020800, X4_4));
    if (EventFlag(12022815)) { //Bear Swap Primer Flag
        SetNetworkconnectedEventFlagID(12022816, ON); //Bear Transform Teleport/Failsafe Primed Flag
    } else {
        SetNetworkconnectedEventFlagID(12022816, OFF); //Bear Transform Teleport/Failsafe Primed Flag
    }
    //DisableLockOnPoint(12020800, 220);
    DisableCharacterGravity(12020800);
    DisableCharacterCollision(12020800);
    DisableCharacterAI(12020800);
    EnableCharacter(12020803);
    EnableCharacter(12020807);
    SetNetworkUpdateRate(12020803, true, CharacterUpdateFrequency.AlwaysUpdate);
    SetNetworkUpdateRate(12020807, true, CharacterUpdateFrequency.AlwaysUpdate);
    
    if (EventFlag(12022816)) { //Bear Transform Teleport/Failsafe Primed Flag
        WarpCharacterAndCopyFloor(12020803, TargetEntityType.Area, 12022812, -1, 12022812);
        WarpCharacterAndCopyFloor(12020807, TargetEntityType.Character, 12020800, 260, 12020800);
    } else {
        WarpCharacterAndCopyFloor(12020803, TargetEntityType.Character, 12020800, 260, 12020800);
        WarpCharacterAndCopyFloor(12020807, TargetEntityType.Area, 12022812, -1, 12022812);
    }
    
    ForceAnimationPlayback(12020803, X8_4, false, false, true);
    ForceAnimationPlayback(12020807, X8_4, false, false, true);
    //WaitFixedTimeFrames(1);
    ForceAnimationPlayback(12020800, 3037, false, false, true);
    
    if (EventFlag(12022816)) { //Bear Transform Teleport/Failsafe Primed Flag
        WaitFor(CharacterHasSpEffect(12020807, X4_4));
    } else {
        WaitFor(CharacterHasSpEffect(12020803, X4_4));
    }
    
    ForceAnimationPlayback(12020800, 3025, false, false, false);
    WaitFixedTimeFrames(1);
    //EnableLockOnPoint(12020800, 220);
    EnableCharacterCollision(12020800);
    EnableCharacterGravity(12020800);
    EnableCharacterAI(12020800);
    
    if (EventFlag(12022816)) { //Bear Transform Teleport/Failsafe Primed Flag
        IssueShortWarpRequest(12020800, TargetEntityType.Character, 12020807, X0_4);
    } else {
        IssueShortWarpRequest(12020800, TargetEntityType.Character, 12020803, X0_4);
    }
    
    RequestCharacterAIReplan(12020800);
    ForceAnimationPlayback(12020803, X12_4, false, false, false);
    ForceAnimationPlayback(12020807, X12_4, false, false, false);
    WaitFixedTimeFrames(2);
    WarpCharacterAndCopyFloor(12020803, TargetEntityType.Area, 12022813, -1, 12022813);
    WarpCharacterAndCopyFloor(12020807, TargetEntityType.Area, 12022813, -1, 12022813);
    RestartEvent();
});

//Transform Teleporting (Birb) - Enemy, SpEffect, Anim, Idle Anim
//For Camera Lock On
$Event(12020553, Restart, function(chrEntityId, dummypolyId) {
    EndIf(EventFlag(12020800));
    DisableNetworkSync();
    WaitFor(CharacterHasSpEffect(chrEntityId, 11744));
    WarpCharacterAndCopyFloor(12020800, TargetEntityType.Character, chrEntityId, dummypolyId, chrEntityId);
    RestartEvent();
});

//Caimar Bear Swap on Grab Success Event Flag Handler (Primer Flag)
$Event(12020554, Restart, function() {
    EndIf(EventFlag(12020800));
    DisableNetworkSync();
    WaitFor(CharacterHasSpEffect(12020803, 11747) || CharacterHasSpEffect(12020807, 11747));
    if (CharacterHasSpEffect(12020803, 11747)) {
        SetNetworkconnectedEventFlagID(12022815, ON); //Bear 1 (803) turns it on for successful grab
        WaitFor(!CharacterHasSpEffect(12020803, 11747));
    } else {
        SetNetworkconnectedEventFlagID(12022815, OFF); //Bear 2 (807) turns it off for successful grab
        WaitFor(!CharacterHasSpEffect(12020807, 11747));
    }
    RestartEvent();
});

//Caimar: Wolf Summon (Left) - Enemy, SpEffect, Attack Anim 1, Attack Anim 2
// Wolf Entity ID 12020808
// Does Anim 3004 (jumps left)
// SpEffect 11748
$Event(12020555, Restart, function(chrEntityId, spEffectId, animationId) {
    DisableLockOnPoint(chrEntityId, 220);
    DisableCharacterGravity(chrEntityId);
    DisableCharacterCollision(chrEntityId);
    DisableCharacter(chrEntityId);
    EndIf(EventFlag(12020800));
    DisableNetworkSync();
    EnableCharacterImmortality(chrEntityId);
    WaitFor(CharacterHasSpEffect(12020800, spEffectId));
    EnableCharacter(chrEntityId);
    SetNetworkUpdateRate(chrEntityId, true, CharacterUpdateFrequency.AlwaysUpdate);
    EnableCharacterAI(chrEntityId);
    WaitFixedTimeFrames(1);
    WarpCharacterAndCopyFloor(chrEntityId, TargetEntityType.Character, 12020800, 260, 12020800);
    RotateCharacter(chrEntityId, 10000, -1, false);
    ForceAnimationPlayback(chrEntityId, animationId, false, false, false);
    WaitFor(CharacterHasSpEffect(chrEntityId, spEffectId));
    WaitFixedTimeFrames(1);
    WaitFor(!CharacterHasSpEffect(chrEntityId, spEffectId));
    RestartEvent();
});

//Caimar: Bear Trap
$Event(12020556, Restart, function(chrEntityId, spEffectId) {
    EndIf(EventFlag(12020800));
    DisableNetworkSync();
    WaitFor(!InArea(chrEntityId, 12022807));
    animationFailure = ElapsedFrames(60);
    WaitFor(CharacterHasSpEffect(chrEntityId, 11744) || animationFailure);
    if (animationFailure.Passed) {
        SetSpEffect(chrEntityId, 10124); // Fade
        WaitFixedTimeFrames(19);
        SetSpEffect(chrEntityId, spEffectId);
    }
    WaitFor(InArea(chrEntityId, 12022807));
    WaitFixedTimeFrames(1);
    ClearSpEffect(chrEntityId, 10124); // Fade
    RestartEvent();
});

//Transform Teleporting (Bear) - Enemy, SpEffect, Anim, Idle Anim
//For Camera Lock On
$Event(12020557, Restart, function(X0_4, X4_4) {
    EndIf(EventFlag(12020800));
    DisableNetworkSync();
    WaitFor(CharacterHasSpEffect(12020807, 11744) || CharacterHasSpEffect(12020803, 11744));
    GotoIf(S0, !EventFlag(12022816)); //Bear Transform Teleport/Failsafe Primed Flag
    WarpCharacterAndCopyFloor(12020800, TargetEntityType.Character, 12020807, X4_4, 12020807);
    RestartEvent();
    Goto(S1);
S0:
    WarpCharacterAndCopyFloor(12020800, TargetEntityType.Character, 12020803, X4_4, 12020803);
    RestartEvent();
S1:
    NoOp();
});

//Caimar: Wolf Summon (Right) - Enemy, SpEffect, Attack Anim 1, Attack Anim 2
// Wolf Entity ID 12020804
// Does Anim 3005 (jumps right)
// SpEffect 11741
$Event(12020558, Restart, function(chrEntityId, spEffectId, animationId) {
    DisableLockOnPoint(chrEntityId, 220);
    DisableCharacterGravity(chrEntityId);
    DisableCharacterCollision(chrEntityId);
    DisableCharacter(chrEntityId);
    EndIf(EventFlag(12020800));
    DisableNetworkSync();
    EnableCharacterImmortality(chrEntityId);
    WaitFor(CharacterHasSpEffect(12020800, spEffectId));
    EnableCharacter(chrEntityId);
    SetNetworkUpdateRate(chrEntityId, true, CharacterUpdateFrequency.AlwaysUpdate);
    EnableCharacterAI(chrEntityId);
    WaitFixedTimeFrames(1);
    WarpCharacterAndCopyFloor(chrEntityId, TargetEntityType.Character, 12020800, 260, 12020800);
    RotateCharacter(chrEntityId, 10000, -1, false);
    ForceAnimationPlayback(chrEntityId, animationId, false, false, false);
    WaitFor(CharacterHasSpEffect(chrEntityId, spEffectId));
    WaitFixedTimeFrames(1);
    WaitFor(!CharacterHasSpEffect(chrEntityId, spEffectId));
    RestartEvent();
});

//Caimar: Airborne Throw Camera Adjustment
$Event(12020559, Restart, function() {
    DisableNetworkSync();
    WaitFor(EventFlag(12020800) || CharacterHasSpEffect(12020800, 11749));
    if (EventFlag(12020800)) {
        ChangeCamera(-1, -1);
        EndEvent();
    }
    ChangeCamera(-1, 4850);
    WaitFor(EventFlag(12020800) || !CharacterHasSpEffect(12020800, 11749));
    ChangeCamera(-1, -1);
    RestartEvent();
});

//Bridge Crystals - EventFlag, Crystal, Bridge Piece 1, Bridge Piece 2, VFX
$Event(12020610, Restart, function(eventFlagId, assetEntityId, mapPartEntityId, mapPartEntityId2, entityId) {
    if (EventFlag(eventFlagId)) {
        RequestAssetDestruction(assetEntityId, 0);
        DeleteAssetfollowingSFX(assetEntityId, false);
        DisableMapPart(mapPartEntityId);
        DisableMapPart(mapPartEntityId2);
        DeleteMapSFX(entityId, true);
        SpawnMapSFX(entityId);
        EndEvent();
    }
    CreateAssetfollowingSFX(assetEntityId, 200, 650014);
    DeleteMapSFX(entityId, true);
    DisableAsset(mapPartEntityId);
    DisableAsset(mapPartEntityId2);
    EnableAssetInvunerability(assetEntityId);
L0:
    if (PlayerIsInOwnWorld()) {
        WaitFor(ActionButtonInArea(6231, assetEntityId) && PlayerIsInOwnWorld());
        RotateCharacter(10000, assetEntityId, -1, true);
        ForceAnimationPlayback(10000, 60010, false, false, false);
        WaitFixedTimeSeconds(1.5);
        SetNetworkconnectedEventFlagID(eventFlagId, ON);
        DisableAssetInvunerability(assetEntityId);
        RequestAssetDestruction(assetEntityId, 0);
        SpawnOneshotSFX(TargetEntityType.Asset, assetEntityId, 200, 650015);
    }
L10:
    WaitFor(EventFlag(eventFlagId));
    DeleteAssetfollowingSFX(assetEntityId, false);
    WaitFixedTimeSeconds(1.3);
    EnableAsset(mapPartEntityId);
    EnableAsset(mapPartEntityId2);
    DisableMapPart(mapPartEntityId);
    DisableMapPart(mapPartEntityId2);
    DeleteMapSFX(entityId, true);
    SpawnMapSFX(entityId);
    EndEvent();
});

// Ghost Animal Ambush - Enemy, Zone, Anim, Anim Lenght (frames), randomTimeStart, randomTimeEnd
$Event(12020680, Restart, function(chrEntityId, areaEntityId, animationId, timeFrames, minTimeFrames, maxTimeFrames, dummypolyId) {
    DisableCharacter(chrEntityId);
    DisableLockOnPoint(chrEntityId, 220);
    EnableCharacterFadeOnEnable(chrEntityId);
    EnableCharacterInvincibility(chrEntityId);
    WaitFor(InArea(10000, areaEntityId));
    WaitRandomTimeFrames(minTimeFrames, maxTimeFrames);
    EnableCharacter(chrEntityId);
    if (Signed(dummypolyId) > 0) {
        IssueShortWarpRequest(chrEntityId, TargetEntityType.Character, 10000, dummypolyId);
    }
    RotateCharacter(chrEntityId, 10000, -1, false);
    ForceAnimationPlayback(chrEntityId, animationId, false, false, true);
    PlaySE(chrEntityId, SoundType.CharacterMotion, 999997800);
    WaitFixedTimeFrames(timeFrames);
    SetSpEffect(chrEntityId, 10124); // Fade
    PlaySE(chrEntityId, SoundType.CharacterMotion, 999997801);
    WaitFixedTimeFrames(22);
    DisableCharacter(chrEntityId);
    DisableCharacterCollision(chrEntityId);
    ForceCharacterDeath(chrEntityId, false);
});

// Spirit Summon for NPC
$Event(12020690, Restart, function(X0_4, X4_4) {
    if (EventFlag(12020425)) {
        DisableCharacter(X0_4);
        DisableCharacterCollision(X0_4);
        ForceCharacterDeath(X0_4, false);
        EndEvent();
    }
    EndIf(CharacterDead(X0_4));
    EnableCharacterFadeOnEnable(X0_4);
    SetSpEffect(X0_4, 10124);
    EnableCharacterInvincibility(X0_4);
    DisableCharacterAI(X0_4);
    WaitFor(InArea(10000, 12022425));
    WaitFixedTimeSeconds(6);
    DisableCharacterInvincibility(X0_4);
    //IssueShortWarpRequest(X0_4, TargetEntityType.Character, 12020425, X4_4);
    RotateCharacter(X0_4, 12022425, -1, false);
    ClearSpEffect(X0_4, 10124);
    WaitFixedTimeFrames(2);
    SpawnOneshotSFX(TargetEntityType.Character, X0_4, 200, 661);
    PlaySE(X0_4, SoundType.CharacterMotion, 999997800);
    WaitFor(ElapsedFrames(20) || HasDamageType(X0_4, 10000, DamageType.Unspecified));
    EnableCharacterAI(X0_4);
});

// Wolf Encounters - Enemy, Region, Anim, anim lenght
$Event(12020695, Restart, function(eventFlagId, areaEntityId, animationId, timeFrames, animationId2) {
    if (EventFlag(eventFlagId)) {
        DisableCharacter(eventFlagId);
        DisableCharacterCollision(eventFlagId);
        ForceCharacterDeath(eventFlagId, false);
        EndEvent();
    }
    SetCharacterTeamType(eventFlagId, 0);
    SetSpEffect(eventFlagId, 13145);
    DisableCharacterAI(eventFlagId);
    WaitFor(InArea(10000, areaEntityId));
    ForceAnimationPlayback(eventFlagId, animationId2, false, true, false);
    ForceAnimationPlayback(eventFlagId, animationId, false, false, false);
    WaitFixedTimeFrames(timeFrames);
    SetEventFlagID(eventFlagId, ON);
    SetSpEffect(eventFlagId, 10124);
    SpawnOneshotSFX(TargetEntityType.Character, eventFlagId, 905, 680000);
    SpawnOneshotSFX(TargetEntityType.Character, eventFlagId, 960, 680000);
    PlaySE(eventFlagId, SoundType.CharacterMotion, 800020000);
    WaitFixedTimeFrames(22);
    DisableCharacter(eventFlagId);
    DisableCharacterCollision(eventFlagId);
    ForceCharacterDeath(eventFlagId, false);
});
