...
L0:
    ...
    WaitFor(EntityInRadiusOfEntity(20000, 21011576, 15, 1));
...
$Event(21012800, Restart, function() {
    ...
    DisableHit(21013891);
    ChangeCamera(-1, -1);
    SetEventFlagID(21010800, ON);
    SetEventFlagID(9146, ON);
    if (PlayerIsInOwnWorld()) {
        SetEventFlagID(61146, ON);
    }
    if (EventFlag(72120)) {
        SetEventFlagID(78901, ON);
    }
    if (EventFlag(76940)) {
        SetEventFlagID(78902, ON);
    }
    if (EventFlag(76941)) {
        SetEventFlagID(78903, ON);
    }
    if (EventFlag(76942)) {
        SetEventFlagID(78904, ON);
    }
    if (EventFlag(76943)) {
        SetEventFlagID(78905, ON);
    }
    if (EventFlag(76945)) {
        SetEventFlagID(78906, ON);
    }
});

//Messmer Pre-Fight Initiation + Cutscene and Fight Initiation
...
L2:
    ...
    SetSpEffect(21010110, 9532); //Entity ID unused
    WaitFixedTimeSeconds(0.5);
    DisplayBossHealthBar(Enabled, 21010801, 0, 905130000);
});

//Messmer Phase 2 Transition Event
...
//Messmer Avoid Fall Death Failsafe
//Warps Messmer into his arena if he ever falls through the floor
...
//Messmer Giant Snake Attacks (spEffectId 20010650 - 20010654, dummypolyId 50, animationId 3000 - 3004)
//Giant Snake <Model ID 5140, Entity Id 21010810>
//Messmer <Model ID 5130, Entity Id 21010800>
//Messmer uses speffects given in anims 20000 - 20004 for summoning the snake for attacks 3000 - 3004
$Event(21012820, Restart, function(spEffectId, dummypolyId, animationId) {
    EndIf(EventFlag(21010800));
    WaitFor(
        !InArea(21010800, 21012819) // Checks Messmer not underground, Snake Available, and Messmer end anim SpEffects
            && EventFlag(21012802)
            && !EventFlag(21013820)
    ...
    if (!CharacterHasSpEffect(21010800, 20010652)) {
        ...
        SetNetworkconnectedEventFlagID(21013820, ON); //Giant Snake Available Flag On (On means Snake is active)
    } else {
L0:
...
//Warp Messmer after Giant Snake Attack (spEffectId 20010660 - 20010664, dummypolyId 20, animationId 20010 - 20014)
//Giant Snake <Model ID 5140, Entity Id 21010810>
//Messmer <Model ID 5130, Entity Id 21010800>
//Giant Snake uses speffects 20010660 - 20010664 given in anims 3000 - 3004 to warp Messmer back
$Event(21012825, Restart, function(spEffectId, dummypolyId, animationId) {
    EndIf(EventFlag(21010800));
    WaitFor(
        !InArea(21010800, 21012819) // Checks Messmer not underground, Snake Available, and Snake end anim SpEffects
            && EventFlag(21013820)
            && CharacterHasSpEffect(21010810, spEffectId));
    WarpCharacterAndCopyFloor(21010800, TargetEntityType.Character, 21010810, dummypolyId, 21010810); //Warps in place of Giant Snake
    if (!InArea(21010800, 21012825)) {
        WarpCharacterAndCopyFloor(21010800, TargetEntityType.Area, 21012815, -1, 21010800); //Failsafe if Messmer doesn't warp back into his arena
    }
L0:
    EnableCharacterCollision(21010800);
    EnableCharacterAI(21010800);
    DisableCharacterCollision(21010810);
    SetNetworkconnectedEventFlagID(21013820, OFF); //Giant Snake Available Flag Off (Off means Snake isn't active)
...
//Messmer Little Snake Attacks (spEffectId 20010642 - 20010647, dummypolyId 42 - 47, animationId 30002 - 30007, chrEntityId 21010822 - 21010827)
//Summons many little snakes during Messmer anims 20023, 20024
...
//Messmer Little Snake Post-Attack Disabler
//SpEffectId 20010649 applied to little snakes at the end of all little snake attacks <AnimationIds 3002 - 3007>
...
//Constant Warp Messmer to Giant Snake while Giant Snake Active
//Used for maintaining camera lock on while all Giant Snake animations use chractionflag 49 (Disable Lock On)
...
//Messmer Base Serpent Snake Attack Camera Adjustement Handler (SpEffectParam, LockCamParam, Entity ID)
...
L2:
    ...
    WaitFor(EventFlag(21010800) || !CharacterHasSpEffect(chrEntityId, spEffectId));
    //Fix for mystery issue of camera not reverting to normal in phase 2
    ChangeCamera(-1, -1);
    RestartEvent();
});

//Player Near Entrance SpEffect Handler
//SpEffectId 20010635 used to limit attacks by setting certain probabilities to 0 when the player is near the entrance of the arena
//Same if logic accounts for event 21012846 and the SpEffects assigned by Messmer being near the entrance
...
//Messmer Death From Hitting Giant Snake
//SpEffectId 20010616 applies 20 sec 1 dmg per .1 sec to Messmer to "force" kill after dealing damage to the Giant Snake while it's at 1 hp
//Messmer cannot die while Giant Snake is active, so it applies the speffect to guarantee his death after he becomes vulnerable again
...
//Messmer Near Entrance SpEffect Handler
//SpEffects set on Messmer adjust probabilities to limit moveset based on whether or not Messmer is near the entrance of his arena
//Same if logic accounts for event 21012844 and the SpEffects assigned by the player being near the entrance
...
//Gparam Override for Fog once player enters region just outside Messmer's arena
//Used for Messmer Fight Entrance Cutscene
...
//Messmer Standard Boss Scripts <Fogwalls, Music, etc.>
